      ...
      override protected function core(x:Array, len:uint) : Array
      {
         ...
         for(var i:* = 0; i < x.length; i = uint(i + 16))
         {
            ...
            for(j = 0; j < 64; j++)
            {
               ...
               {
                  s0 = uint(this.rrol(w[j - 15],7) ^ this.rrol(w[j - 15],18) ^ w[j - 15] >>> 3);
                  s1 = uint(this.rrol(w[j - 2],17) ^ this.rrol(w[j - 2],19) ^ w[j - 2] >>> 10);
                  w[j] = w[j - 16] + s0 + w[j - 7] + s1;
               }
               t2 = uint((this.rrol(a,2) ^ this.rrol(a,13) ^ this.rrol(a,22)) + (a & b ^ a & c ^ b & c));
               t1 = uint(h + (this.rrol(e,6) ^ this.rrol(e,11) ^ this.rrol(e,25)) + (e & f ^ g & ~e) + k[j] + w[j]);
               h = g;
               g = f;
               f = e;
               e = d + t1;
