package com.hurlant.crypto.symmetric
{
   ...
   public class DESKey implements ISymmetricKey
   {
      ...
      protected function desFunc(wKey:Array, inp:ByteArray, inOff:uint, out:ByteArray, outOff:uint) : void
      {
         ...
         var left:* = 0;
         var fval:* = 0;
         left = uint((inp[inOff + 0] & 255) << 24);
         left = uint(left | (inp[inOff + 1] & 255) << 16);
         left = uint(left | (inp[inOff + 2] & 255) << 8);
         left = uint(left | inp[inOff + 3] & 255);
         right = uint((inp[inOff + 4] & 255) << 24);
         right = uint(right | (inp[inOff + 5] & 255) << 16);
         right = uint(right | (inp[inOff + 6] & 255) << 8);
         right = uint(right | inp[inOff + 7] & 255);
         work = uint((left >>> 4 ^ right) & 252645135);
         right = uint(right ^ work);
         left = uint(left ^ work << 4);
         work = uint((left >>> 16 ^ right) & 65535);
         right = uint(right ^ work);
         left = uint(left ^ work << 16);
         work = uint((right >>> 2 ^ left) & 858993459);
         left = uint(left ^ work);
         right = uint(right ^ work << 2);
         work = uint((right >>> 8 ^ left) & 16711935);
         left = uint(left ^ work);
         right = uint(right ^ work << 8);
         right = uint((right << 1 | right >>> 31 & 1) & 4294967295);
         work = uint((left ^ right) & 2863311530);
         left = uint(left ^ work);
         right = uint(right ^ work);
         left = uint((left << 1 | left >>> 31 & 1) & 4294967295);
         for(var round:int = 0; round < 8; round++)
         {
            work = uint(right << 28 | right >>> 4);
            work = uint(work ^ wKey[round * 4 + 0]);
            fval = uint(SP7[work & 63]);
            fval = uint(fval | SP5[work >>> 8 & 63]);
            fval = uint(fval | SP3[work >>> 16 & 63]);
            fval = uint(fval | SP1[work >>> 24 & 63]);
            work = uint(right ^ wKey[round * 4 + 1]);
            fval = uint(fval | SP8[work & 63]);
            fval = uint(fval | SP6[work >>> 8 & 63]);
            fval = uint(fval | SP4[work >>> 16 & 63]);
            fval = uint(fval | SP2[work >>> 24 & 63]);
            left = uint(left ^ fval);
            work = uint(left << 28 | left >>> 4);
            work = uint(work ^ wKey[round * 4 + 2]);
            fval = uint(SP7[work & 63]);
            fval = uint(fval | SP5[work >>> 8 & 63]);
            fval = uint(fval | SP3[work >>> 16 & 63]);
            fval = uint(fval | SP1[work >>> 24 & 63]);
         ...
         work = uint((left >>> 8 ^ right) & 16711935);
         right = uint(right ^ work);
         left = uint(left ^ work << 8);
         work = uint((left >>> 2 ^ right) & 858993459);
         right = uint(right ^ work);
         left = uint(left ^ work << 2);
         work = uint((right >>> 16 ^ left) & 65535);
         left = uint(left ^ work);
         right = uint(right ^ work << 16);
         work = uint((right >>> 4 ^ left) & 252645135);
         left = uint(left ^ work);
         right = uint(right ^ work << 4);
