package com.ankama.chat
{
   ...
   public final dynamic class Roster extends Message
   {
      ...
      public static const GROUPS:RepeatedFieldDescriptor_TYPE_STRING = new RepeatedFieldDescriptor_TYPE_STRING("com.ankama.chat.Roster.groups","groups",8 | WireType.LENGTH_DELIMITED);
      
      public static const FRIENDS:RepeatedFieldDescriptor_TYPE_MESSAGE = new RepeatedFieldDescriptor_TYPE_MESSAGE("com.ankama.chat.Roster.friends","friends",16 | WireType.LENGTH_DELIMITED,function():Class
      {
         return Friend;
      });
      
      public static const INVITES:RepeatedFieldDescriptor_TYPE_MESSAGE = new RepeatedFieldDescriptor_TYPE_MESSAGE("com.ankama.chat.Roster.invites","invites",24 | WireType.LENGTH_DELIMITED,function():Class
      {
         return InviteSent;
      });
      ...
      override final function writeToBuffer(output:WritingBuffer) : void
      {
         ...
         for(var groups$index:int = 0; groups$index < this.groups.length; groups$index++)
         {
            WriteUtils.writeTag(output,WireType.LENGTH_DELIMITED,1);
            WriteUtils.write_TYPE_STRING(output,this.groups[groups$index]);
         }
         for(var friends$index:int = 0; friends$index < this.friends.length; friends$index++)
         {
            WriteUtils.writeTag(output,WireType.LENGTH_DELIMITED,2);
            WriteUtils.write_TYPE_MESSAGE(output,this.friends[friends$index]);
         }
         for(var invites$index:int = 0; invites$index < this.invites.length; invites$index++)
         {
