package com.ankamagames.jerakine.pathfinding
{
   ...
   public class Pathfinding
   {
      ...
      private var _avoidObstacles:Boolean;
      ...
      public static function findPath(map:IDataMapProvider, start:MapPoint, end:MapPoint, allowDiag:Boolean = true, bAllowTroughEntity:Boolean = true, callBack:Function = null, args:Array = null, bIsFighting:Boolean = false, avoidObstacles:Boolean = true) : MovementPath
      {
         return new Pathfinding().processFindPath(map,start,end,allowDiag,bAllowTroughEntity,callBack,args,bIsFighting,avoidObstacles);
      }
      
      public function processFindPath(map:IDataMapProvider, start:MapPoint, end:MapPoint, allowDiag:Boolean = true, bAllowTroughEntity:Boolean = true, callBack:Function = null, args:Array = null, bIsFighting:Boolean = false, avoidObstacles:Boolean = true) : MovementPath
      {
         ...
         this._avoidObstacles = avoidObstacles;
      ...
      private function nearerSquare() : uint
      {
         ...
         var indexFound:* = 0;
         var i:int = -1;
         for(var len:int = this._openList.length; ++i < len; )
         {
      ...
      {
         var pElem:* = null;
         for(var i:int = 0; i < returnPath.length - 1; i++)
         {
      ...
      private function pathFrame(E:Event) : void
      {
         ...
         if(this._openList.length > 0 && !this.isClosed(this._endY,this._endX))
         {
            ...
            {
               for(i = this._nowX - 1; i < this._nowX + 2; )
               {
                  if(j >= _minY && j < _maxY && i >= _minX && i < _maxX && !(j == this._nowY && i == this._nowX) && (this._allowDiag || j == this._nowY || i == this._nowX && (this._bAllowDiagCornering || j == this._nowY || i == this._nowX || (this._map.pointMov(this._nowX,j,this._bAllowTroughEntity,this._previousCellId,this._endPoint.cellId,this._avoidObstacles) || this._map.pointMov(i,this._nowY,this._bAllowTroughEntity,this._previousCellId,this._endPoint.cellId,this._avoidObstacles)))))
                  {
                     ...
                     {
                        if(this._map.pointMov(i,j,this._bAllowTroughEntity,this._previousCellId,this._endPoint.cellId,this._avoidObstacles))
                        {
                           if(!this.isClosed(j,i))
                           {
                              ...
                              {
                                 pointWeight = Number(this._map.pointWeight(i,j,this._bAllowTroughEntity));
                              }
      ...
      private function endPathFrame() : void
      {
         ...
         var newY:int = 0;
         var newX:int = 0;
         var tmpMapPoint:* = null;
         var returnPathOpti:* = null;
         var k:int = 0;
         var kX:int = 0;
         var kY:int = 0;
         var nextX:int = 0;
            ...
            if(this._allowDiag)
            {
               ...
               for(k = 0; k < returnPath.length; )
               {
                  ...
                  else if(returnPath[k + 2] && MapPoint(returnPath[k]).distanceToCell(returnPath[k + 2]) == 2)
                  {
                     ...
                     else if(kX == nextX && kX != interX && this._map.pointWeight(kX,interY) < 2 && this._map.pointMov(kX,interY,this._bAllowTroughEntity,this._previousCellId,this._endPoint.cellId))
                     {
                     ...
                     else if(kY == nextY && kY != interY && this._map.pointWeight(interX,kY) < 2 && this._map.pointMov(interX,kY,this._bAllowTroughEntity,this._previousCellId,this._endPoint.cellId))
                     {
      ...
      private function tracePath(returnPath:Array) : void
      {
         ...
         for(var i:int = 0; i < returnPath.length; i++)
         {
