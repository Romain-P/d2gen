   ...
   public class GameDataFileAccessor
   {
      ...
      public function initFromIDataInput(stream:IDataInput, moduleName:String) : void
      {
         ...
         if(headers != "D2O")
         {
            ...
            formatVersion = uint(stream.readShort());
            len = uint(stream.readInt());
            stream["position"] = stream["position"] + len;
            contentOffset = uint(stream["position"]);
         ...
         for(var i:* = 0; i < indexesLength; i = uint(i + 8))
         {
            key = stream.readInt();
         ...
         for(var j:int = 0; j < classesCount; j++)
         {
