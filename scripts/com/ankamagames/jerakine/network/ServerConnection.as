package com.ankamagames.jerakine.network
{
   ...
   public class ServerConnection implements IEventDispatcher, IServerConnection
   {
      ...
      private static const MESSAGE_SIZE_ASYNC_THRESHOLD:uint = 307200;
      ...
      private function process(msg:INetworkMessage) : void
      {
         ...
         {
            this.receive(this._asyncNetworkDataContainerMessage.content);
         }
         else
         {
      ...
      private function readMessageLength(staticHeader:uint, src:IDataInput) : uint
      {
         ...
         switch(byteLenDynamicHeader)
         {
            case 0:
               break;
            case 1:
               messageLength = uint(src.readUnsignedByte());
               break;
            case 2:
               messageLength = uint(src.readUnsignedShort());
               break;
            case 3:
               messageLength = uint(((src.readByte() & 255) << 16) + ((src.readByte() & 255) << 8) + (src.readByte() & 255));
         }
      ...
      protected function lowReceive(src:IDataInput) : INetworkMessage
      {
         ...
         if(!this._splittedPacket)
         {
            ...
            staticHeader = uint(src.readUnsignedShort());
            messageId = uint(this.getMessageId(staticHeader));
            if(src.bytesAvailable >= (staticHeader & NetworkMessage.BIT_MASK))
            {
               messageLength = uint(this.readMessageLength(staticHeader,src));
