package com.ankamagames.atouin.renderers
{
   ...
   public class TacticalModeRenderer implements IFightZoneRenderer
   {
      ...
      public function render(cells:Vector.<uint>, oColor:Color, mapContainer:DataMapContainer, alpha:Boolean = false, updateStrata:Boolean = false) : void
      {
         ...
         for each(tacticModeCell in this._cellsToRender)
         {
            for each(elem in tacticModeCell.elements)
            {
               ...
               if(!isJpg && Atouin.getInstance().options.pngPathOverride)
               {
                  path = Atouin.getInstance().options.pngPathOverride + "/" + elementData.gfxId + ".png";
               }
               else
               {
                  path = Atouin.getInstance().options.elementsPath + "/" + (!!isJpg?Atouin.getInstance().options.jpgSubPath:Atouin.getInstance().options.pngSubPath) + "/" + elementData.gfxId + "." + (!!isJpg?"jpg":Atouin.getInstance().options.mapPictoExtension);
               }
      ...
      private function onAllGfxLoaded(e:ResourceLoaderProgressEvent) : void
      {
         ...
         var w:Number = NaN;
         this._displayedCells = new Vector.<TacticalModeRenderer>(0);
               ...
               for(i = 0; i < numCells; )
               {
                  ...
                  if(!this.isReachable(bottomCell) && this._cells.indexOf(bottomCell.cellId) == -1)
                  {
                     ...
                     if(leftVisible)
                     {
                        ...
                        offset = Number(rightVisible && this.isReachable(mp)?0:Number(2));
                        ...
                        hoffset = Number(offset);
                        if(offset != 0 && (this.isReachable(mp) || mp && this._cells.indexOf(mp.cellId) != -1))
                        {
                           hoffset = 0;
                        }
                        offset = Number(!rightVisible && !skipOffset?Number(offset):Number(0));
                     ...
                     {
                        offset2 = !!this.isReachable(MapPoint.fromCoords(bottomCell.x + 1,bottomCell.y - 1))?-2:Number(0);
