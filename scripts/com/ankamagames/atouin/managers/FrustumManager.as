package com.ankamagames.atouin.managers
{
   ...
   public class FrustumManager
   {
      ...
      public function findNearestBorderCellFromPoint(direction:int, fromCellPoint:Point) : Object
      {
         ...
         var currentCellPixelPoint:* = null;
         ...
         {
            maxI = uint(AtouinConstants.MAP_HEIGHT * 2);
            for(i = 0; i < maxI; i++)
            {
               ...
               if(currentDistance < lastSmallestDistance)
               {
                  ...
                  mapChangeData = uint(cellData.mapChangeData);
      ...
      private function out(e:MouseEvent) : void
      {
         ...
         {
            case this._shapeRight:
               n = uint(DirectionsEnum.RIGHT);
               break;
            case this._shapeLeft:
               n = uint(DirectionsEnum.LEFT);
               break;
            case this._shapeBottom:
               n = uint(DirectionsEnum.DOWN);
               break;
            case this._shapeTop:
               n = uint(DirectionsEnum.UP);
         }
         this._lastCellId = -1;
