package com.ankamagames.atouin.managers
{
   ...
   public class InteractiveCellManager
   {
      ...
      public function InteractiveCellManager()
      {
         ...
         this._showEveryCellId = Atouin.getInstance().options.showEveryCellId;
         super();
         if(_self)
         {
      ...
      public function updateInteractiveCell(container:DataMapContainer) : void
      {
         ...
         this.showEveryCellId(Atouin.getInstance().options.showEveryCellId);
         var aCell:Array = container.getCell();
         var showTransitions:Boolean = Atouin.getInstance().options.showTransitions;
         var alpha:* = Number(this._bShowGrid || Atouin.getInstance().options.alwaysShowGrid?1:Number(0));
      ...
      public function show(b:Boolean, pIsInFight:Boolean = false) : void
      {
         ...
         for(var i:int = 0; i < this._aCells.length; i++)
         {
      ...
      {
         var c:* = null;
         for(var i:int = 0; i < AtouinConstants.MAP_CELLS_COUNT; i++)
         {
            c = new GraphicCell(i);
