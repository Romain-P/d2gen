package com.ankamagames.berilia.types.tooltip
{
   ...
   public class TooltipPlacer
   {
      ...
      public static function place(tooltip:DisplayObject, target:IRectangle, showDirectionalArrowInfo:Object, point:uint = 6, relativePoint:uint = 0, offset:* = 3, alwaysDisplayed:Boolean = true) : void
      {
         ...
         var pTarget:* = null;
         var pTooltip:* = null;
         var hackIRectangle:* = null;
         var offsetPt:* = null;
         var tooltipZone:* = null;
         var hitZoneSize:int = 0;
         var newPt:* = null;
         var smallerZone:* = null;
         var obj:* = null;
         var arrowInfos:* = null;
         var arrow:* = null;
         ...
         for(var screenBounds:Rectangle = StageShareManager.stageVisibleBounds; !ok; )
         {
            ...
            if(i + 1 < len)
            {
               ...
               if(rowLen > 1)
               {
                  ...
                  for(j = i + 1; j < len; )
                  {
                     ...
                     {
                        minTooltipY = Number(tooltipY - tooltipHeight - 2);
                        break;
                     }
                     j++;
                  }
                  minX = Number(maxX = Number(_tooltips[i].target.x));
                  for each(tooltipPosition in _tooltipsRows[currentTooltipRow])
                  {
                     tooltipPosition.y = Math.max(0,minTooltipY);
                     if(tooltipPosition.target.x < minX)
                     {
                        minX = Number(tooltipPosition.target.x);
                     }
                     else if(tooltipPosition.target.x > maxX)
                     {
                        maxX = Number(tooltipPosition.target.x);
                     }
                     rowWidth = Number(rowWidth + tooltipPosition.width);
                  }
                  _tooltipsRows[currentTooltipRow].sort(compareHorizontalPos);
                  rowWidth = Number(rowWidth + 2 * (rowLen - 1));
                  centerX = (maxX + minX) / 2;
      ...
      {
         var tooltipPosition:* = null;
         var tooltipPositionsInCollision:* = null;
