package com.ankamagames.berilia.types.graphic
{
   ...
   public class ResizeController extends Sprite implements MessageHandler
   {
         ...
         if(change)
         {
            ...
            try
            {
               ...
               if(gc.getUi().windowOwner)
               {
                  gc.xNoCache = 0;
                  gc.yNoCache = 0;
                  gc.getUi().windowOwner.width = gc.width + 20;
                  gc.getUi().windowOwner.height = gc.height + 20;
               }
               return;
            }
            catch(err:Error)
      ...
      private function resizeEnd() : void
      {
         ...
         {
            this.targetContainer.yNoCache = 0;
            this.targetContainer.xNoCache = 0;
            this.targetContainer.getUi().windowOwner.width = this.targetContainer.width + 20;
            this.targetContainer.getUi().windowOwner.height = this.targetContainer.height + 20;
         }
      }
      ...
      private function configShape(x:int, y:int, width:int, height:int, target:UiBorder = null, resizeH:Boolean = false, resizeV:Boolean = false, cursorId:String = null) : UiBorder
      {
         ...
         {
            target = new UiBorder();
            target.graphics.beginFill(16777215 * Math.random(),!!DEBUG_SHAPE?0.8:Number(0));
