package com.ankamagames.berilia.types.graphic
{
   ...
   public class UiRootContainer extends GraphicContainer implements UIComponent
   {
      ...
      public function UiRootContainer(stage:Stage, uiData:UiData, root:Sprite = null)
      {
         ...
         MEMORY_LOG["[object UiRootContainer]"] = 1;
      }
      ...
      public function render() : void
      {
         ...
         this.processSize();
         for(i = 0; i < this._aGraphicLocationStack.length; )
         {
      ...
      public function removeFromRenderList(sName:String) : void
      {
         ...
         for(i = 0; i < this._aSizeStack.length; )
         {
      ...
      public function addDynamicSizeElement(geReference:GraphicElement) : void
      {
         ...
         for(var i:int = 0; i < this._aSizeStack.length; )
         {
      ...
      {
         for(var i:int = 0; i < this._aGraphicLocationStack.length; )
         {
      ...
      private function processSize() : void
      {
         ...
         var p:* = null;
         for(var i:int = 0; i < this._aSizeStack.length; i++)
         {
      ...
      public function processLocation(geElem:GraphicElement, fromRender:Boolean = false) : void
      {
         var ptNewPos:* = null;
         var center:* = null;
         var ptTopLeftCorner:* = null;
         var ptBottomRightCorner:* = null;
         if(!fromRender)
         {
         ...
         if(p)
         {
            if(!geElem.sprite.dragController || geElem.sprite._dragController.savePosition)
            {
               ...
               if(this.fullscreen)
               {
                  center = new Point(StageShareManager.startWidth / 2,StageShareManager.startHeight / 2);
                  if(p.x > center.x)
                  {
                     ptNewPos = new Point(p.x - StageShareManager.stageVisibleBounds.x + this._stageAddedWidth,p.y);
                  }
                  else
                  {
                     ptNewPos = new Point(p.x - StageShareManager.stageVisibleBounds.x - this._stageAddedWidth,p.y);
         ...
         ptNewPos = this.getLocation(new Point(geElem.sprite.x,geElem.sprite.y),geElem.location,geElem.sprite);
         if(geElem.sprite && ptNewPos)
         {
      ...
      private function getOffsetModificator(nPoint:uint, doTarget:DisplayObject) : Point
      {
         ...
         if(doTarget == null || doTarget is UiRootContainer)
         {
            if(this._fullscreen)
            {
               nWidth = uint(Math.abs(StageShareManager.stageVisibleBounds.width));
               nHeight = uint(Math.abs(StageShareManager.stageVisibleBounds.height));
            }
            else
            {
               nWidth = uint(Math.abs(StageShareManager.startWidth));
               nHeight = uint(Math.abs(StageShareManager.startHeight));
            }
         }
         else
         {
            nWidth = uint(Math.abs(doTarget.width));
            nHeight = uint(Math.abs(doTarget.height));
         }
      ...
      private function zSort(aSort:Array) : Boolean
      {
         ...
         while(bChange)
         {
            ...
            for(i = 0; i < aSort.length; i++)
            {
               ...
               if(ge != null)
               {
                  for(j = 0; j < ge.locations.length; )
                  {
                     for(k = uint(i + 1); k < aSort.length; )
                     {
                        gl = ge.locations[j];
                        if(aSort[k] != null)
                        {
