package com.ankamagames.berilia.types.graphic
{
   ...
   public class GraphicContainer extends Sprite implements UIComponent, IRectangle, Poolable, IDragAndDropHandler, ICustomUnicNameGetter, FinalizableUIComponent
   {
      ...
      public function set verticalDrag(b:Boolean) : void
      {
         if(!this.dragController)
         {
            this.dragController = true;
         }
         this._dragController.verticalDrag = b;
      }
      ...
      public function set horizontalDrag(b:Boolean) : void
      {
         if(!this.dragController)
         {
            this.dragController = true;
         }
         this._dragController.horizontalDrag = b;
      }
      ...
      public function set sizeControler(b:Boolean) : void
      {
         ...
         {
            this.getUi().windowOwner.sizeControler = !!b?{}:null;
         }
      }
      ...
      public function set bgColor(nColor:*) : void
      {
         ...
         graphics.beginFill(this.bgColor,this.bgColor == -1?0:Number(this._bgAlpha));
      ...
      protected function setColorVar(nColor:*) : void
      {
         ...
         if(nColor is String)
         {
            base = uint(String(nColor).substr(0,2) == "0x"?16:uint(10));
      ...
      public function getStrata(nStrata:int) : Sprite
      {
         ...
         nIndex = 0;
         for(i = 0; i < this._aStrata.length; )
         {
      ...
      public function get depths() : Array
      {
         ...
         for(var doParent:* = {}; doParent.getParent() != null; )
         {
      ...
      public function getUi() : UiRootContainer
      {
         ...
         var doCurrent:* = Sprite(this.parent);
      ...
      {
         var data:* = StoreDataManager.getInstance().getData(BeriliaConstants.DATASTORE_UI_POSITIONS,this.getUi().name + "##pos##" + name + "##" + Berilia.getInstance().uiSavedModificationPresetName);
      ...
      {
         StoreDataManager.getInstance().setData(BeriliaConstants.DATASTORE_UI_POSITIONS,this.getUi().name + "##pos##" + name + "##" + Berilia.getInstance().uiSavedModificationPresetName,isNaN(x) && isNaN(y)?null:new Point(x,y));
         if(this.getUi())
         {
      ...
      {
         var data:* = StoreDataManager.getInstance().getData(BeriliaConstants.DATASTORE_UI_POSITIONS,this.getUi().name + "##size##" + name + "##" + Berilia.getInstance().uiSavedModificationPresetName);
         if(data && !(data is Point))
         {
            data = new Point(data.x,data.y);
         }
      ...
      public function setSavedDimension(x:Number, y:Number) : void
      {
         ...
         StoreDataManager.getInstance().setData(BeriliaConstants.DATASTORE_UI_POSITIONS,this.getUi().name + "##size##" + name + "##" + Berilia.getInstance().uiSavedModificationPresetName,isNaN(x) && isNaN(y)?null:new Point(x,y));
      }
