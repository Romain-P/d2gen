package com.ankamagames.berilia.components
{
   ...
   public class MapViewer extends GraphicContainer implements FinalizableUIComponent
   {
      ...
      public static const CELL_HALF_WIDTH:Number = 40;
      
      public static const CELL_HALF_HEIGHT:Number = 21;
      ...
      public function get visibleMaps() : Rectangle
      {
         ...
         return new Rectangle(vX,vY,w < 1?1:Number(w),h < 1?1:Number(h));
      }
      ...
      public function addIcon(layer:String, id:String, rawUri:String, x:int, y:int, scale:Number = 1, legend:String = null, follow:Boolean = false, color:int = -1, canBeGrouped:Boolean = true, canBeManuallyRemoved:Boolean = true, hitArea:Rectangle = null, mouseEnabled:Boolean = false, allowDuplicate:Boolean = false, priority:uint = 0) : MapIconElement
      {
         ...
         if(this._layers[layer] && this.mapBounds.contains(x,y))
         {
            ...
            if(!this._elementsListsByCoordinates[mie.key])
            {
               this._elementsListsByCoordinates[mie.key] = [];
            }
            this._elementsListsByCoordinates[mie.key].push(mie);
            return mie;
         }
         return null;
      }
      ...
      public function addLine(layer:String, id:String, fromX:int, fromY:int, toX:int, toY:int, rawUri:String, text:String = "", color:uint = 16711680) : MapIconElement
      {
         ...
         var t:TextureBase = uri.path && uri.fileType.toLowerCase() == "png"?new TextureBitmap():new Texture();
         t.mouseChildren = false;
         t.autoCenterBitmap = true;
         if(t is TextureBitmap)
         {
      ...
      {
         var oldRouteShape:* = null;
         var routeTexture:* = null;
         var graphic:* = null;
         var coordCount:int = 0;
         var i:int = 0;
         var posX:int = 0;
         var posY:int = 0;
         var mapRouteShape:* = null;
         if(this._layers[layer] && coordList)
         {
            ...
            if(oldRouteShape)
            {
               ...
               this._mapElements.splice(this._mapElements.indexOf(oldRouteShape),1);
            }
            ...
            this._mapElements.push(mapRouteShape);
      ...
      {
         var oldAreaShape:* = null;
         var shapeZone:* = null;
         var graphic:* = null;
         var nCoords:int = 0;
         var i:int = 0;
         var mapAreaShape:* = null;
         var posX:int = 0;
         var posY:int = 0;
         if(this._layers[layer] && coordList)
         {
      ...
      {
         var elemKey:String = x + "_" + y;
         if(!this._elementsListsByCoordinates[elemKey])
         {
            return [];
         }
         return (this._elementsListsByCoordinates[elemKey] as Array).sortOn("priority");
      }
      ...
      public function removeMapElement(me:MapElement) : void
      {
         ...
         if(index != -1)
         {
            mie = this._mapElements[index] as MapIconElement;
            if(mie)
            {
               iconTexture = this._elementTexturesByElement[mie];
               if(this._mapGroupElements[mie])
               {
                  this._mapGroupElements[mie].icons.splice(this._mapGroupElements[mie].icons.indexOf(mie),1);
               ...
               if(this._elementsListsByCoordinates[mie.key])
               {
                  index2 = this._elementsListsByCoordinates[mie.key].indexOf(mie);
                  if(index2 != -1)
                  {
                     this._elementsListsByCoordinates[mie.key].splice(index2,1);
                  }
               }
      ...
      private function processUpdateMapElements(e:Event) : void
      {
         ...
         for each(elem in this._mapElements)
         {
            ...
            {
               elemKey = elem.x + "_" + elem.y;
               if(!sortedMapElements[elemKey])
               {
                  sortedMapElements[elemKey] = new Array();
               }
               if(!(elem is MapIconElement) || !this.checkDuplicate(elem as MapIconElement,sortedMapElements[elemKey]))
               {
                  sortedMapElements[elemKey].push(elem);
               }
            }
         }
      ...
      private function checkDuplicate(pElement:MapIconElement, pElements:Array) : Boolean
      {
         var elem:* = null;
         var iconElem:* = null;
         ...
         for each(elem in pElements)
         {
            ...
            if(iconElem && iconElem.legend == pElement.legend)
            {
               return true;
            }
         }
      ...
      private function updateIcons() : void
      {
         ...
         var isCircleView:* = this.roundCornerRadius > width / 3;
         ...
         for each(me in this._mapElements)
         {
            ...
            if(mie)
            {
               ...
               if(iconTexture)
               {
                  if(mie.follow && isCircleView)
                  {
                     p = globalToLocal(this.getIconTextureGlobalCoords(mie));
                     iconVisible = Math.floor(Math.sqrt(Math.pow(p.x - radius,2) + Math.pow(p.y - radius,2))) < radius;
                  }
                  else
                  {
                     iconVisible = Boolean(!iconTexture.uri || !iconTexture.finalized || !iconTexture.parent?Boolean(visibleMaps.intersects(iconRect)):Boolean(globalRect.intersects(mie.getRealBounds)));
                  }
                  iconTexture.visible = this._layers[mie.layer].visible != false && iconVisible;
                  if(mie.group)
                  {
                     mie.group.visible = iconTexture.visible;
                  }
         ...
         for(icon in this._arrowsByElementTexture)
         {
            ...
            else
            {
               ...
               {
                  res = -height / 2 / a + width / 2;
                  res = res > width?Number(width):res < 0?0:Number(res);
                  if(res >= 0)
                  {
                     arrow.x = res;
                     arrow.y = 0;
      ...
      private function updateVisibleChunck(refreshIcons:Boolean = true) : void
      {
         ...
         this._viewRect.x = -this._mapContainer.x / this._mapContainer.scaleX - 100;
         this._viewRect.y = -this._mapContainer.y / this._mapContainer.scaleY - 100;
         this._viewRect.width = width / this._mapContainer.scaleX + 200;
         this._viewRect.height = height / this._mapContainer.scaleY + 200;
      ...
      private function drawGrid() : void
      {
         ...
         else
         {
            ...
            verticalLineCount = uint(this._mapBitmapContainer.width / this.mapWidth);
            ...
            horizontalLineCount = uint(this._mapBitmapContainer.height / this.mapHeight);
      ...
      {
         for(var mapGroupElement:* = null; this._groupsContainer.numChildren > 0; )
         {
      ...
      private function clearMapRouteShapes() : void
      {
         var mapRouteShape:* = null;
         var layer:* = null;
         var i:int = 0;
         var numLayers:int = this._routeShapesContainer.numChildren;
         for(i = 0; i < numLayers; i++)
         {
            layer = this._routeShapesContainer.getChildAt(i) as Sprite;
            while(layer.numChildren)
            {
               mapRouteShape = layer.getChildAt(0) as MapRouteShape;
               mapRouteShape.remove();
               layer.removeChildAt(0);
            }
         }
      }
      ...
      private function updateIconSize() : void
      {
         ...
         while(currentDo && currentDo.parent)
         {
            ...
            realScale = realScale * currentDo.scaleX;
         }
      ...
      private function onMapEnterFrame(e:Event) : void
      {
         ...
         var stageMouseX:Number = !!this._enable3DMode?Number(StageShareManager.mouseX):Number(stage.mouseX);
         var stageMouseY:Number = !!this._enable3DMode?Number(StageShareManager.mouseY):Number(stage.mouseY);
         if(this._dragging && (this._lastMouseX != stageMouseX || this._lastMouseY != stageMouseY))
         {
            newX = this._mapContainer.x + (!!this._enable3DMode?-(stageMouseX - this._lastMouseX):stageMouseX - this._lastMouseX);
            newY = this._mapContainer.y + (!!this._enable3DMode?-(stageMouseY - this._lastMouseY):stageMouseY - this._lastMouseY);
            if(this._lastMouseX && (this._enable3DMode || newX <= 0 && Math.round(newX + this._currentMap.initialWidth * this._mapContainer.scaleX) >= width))
            {
               this._mapContainer.x = newX;
            }
            if(this._lastMouseX && (this._enable3DMode || newY <= 0 && Math.round(newY + this._currentMap.initialHeight * this._mapContainer.scaleY) >= height))
            {
      ...
      {
         var currentMapGroupElement:* = null;
         ...
         var currentMapIcons:Array = this._elementsListsByCoordinates[this._lastMx + "_" + this._lastMy];
