      ...
      override public function process(msg:Message) : Boolean
      {
         ...
         switch(true)
         {
            case msg is KeyboardKeyUpMessage:
               keyCode = uint(KeyboardMessage(msg).keyboardEvent.keyCode);
               ...
               if(handleKey(keyCode) && FocusHandler.getInstance().getFocus() == this.input.textfield)
               {
                  ...
                  if(inputText != "\b" && this._origDataProvider && this._origDataProvider.length > 0)
                  {
                     this.showList(true);
                     break;
                  }
