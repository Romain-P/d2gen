package com.ankamagames.berilia.components
{
   ...
   public class ChatComponent extends GraphicContainer implements UIComponent, IRectangle, FinalizableUIComponent
   {
      ...
      private var _smilies:Vector.<Smiley#475>;
      ...
      public function ChatComponent()
      {
         ...
         this._sbScrollBar.scrollSpeed = 0.166666666666667;
      ...
      public function initSmileyTab(uri:String, data:Object) : void
      {
         ...
         this._smilies = new Vector.<Smiley#475>();
         for each(t in data)
         {
            if(t.triggers != null && t.triggers.length > 0)
            {
               smiley = new Smiley#475(t.gfxId);
      ...
      {
         val = val - this._sbScrollBar.width;
         super.width = val;
      ...
      public function get textHeight() : Number
      {
         ...
         for(i = 0; i < len; i++)
         {
            height = Number(height + this.getParagraphHeight(this._textFlow.getChildAt(i) as ParagraphElement));
         }
         return height;
      }
      ...
      private function getParagraphHeight(p:ParagraphElement) : Number
      {
         ...
         for(var endPos:int = pos + p.textLength; pos < endPos; )
         {
            line = p.getTextFlow().flowComposer.findLineAtPosition(pos);
            height = Number(height + line.height);
      ...
      private function createParagraphe(text:String) : ParagraphElement
      {
         ...
         for(var result:Object = new RegExp(TAGS_PATTERN).exec(text); result != null; )
         {
            if(result.index > 0)
            {
      ...
      private function createSpan(p:ParagraphElement, sText:String, handleHtmlTags:Boolean, pStyle:String = "") : void
      {
         ...
         while(sText.length > 0)
         {
            ...
            if(textToShow.length > 0 || smiley == null)
            {
               ...
               {
                  for(kamaIndex = textToShow.search(KAMA_PATTERN); kamaIndex != -1; )
                  {
      ...
      private function createSpanElementsFromHtmlTags(p:ParagraphElement, pText:String, pStyle:String) : void
      {
         ...
         var attributes:* = null;
         var att:* = null;
         for(result = new RegExp(TAGS_PATTERN).exec(pText); result != null; )
         {
            if(result.index > 0)
            {
      ...
      private function getSmileyFromText(sTxt:String) : Smiley#475
      {
         var indexOfSmiley:int = 0;
         var currentSmiley:* = null;
         var smiley:* = null;
         var trigger:* = null;
...
class Smiley#475
{
   ...
   function Smiley#475(pId:String)
   {
      super();
      this.pictoId = pId;
      this.position = -1;
