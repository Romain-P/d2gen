package com.ankamagames.berilia.components
{
   ...
   public class Label extends GraphicContainer implements UIComponent, IRectangle, FinalizableUIComponent
   {
      ...
      {
         var field:* = null;
         if(_filterIndex[getQualifiedClassName(target)])
         {
            return;
         }
         var index:Dictionary = new Dictionary();
         var fields:Array = DescribeTypeCache.getVariables(target);
         for each(field in fields)
         {
            index[field.toLowerCase()] = field;
         }
         _filterIndex[getQualifiedClassName(target)] = index;
      }
      ...
      {
         var textrun:* = null;
         var textrunText:* = null;
         var openSquareBrackets:* = null;
         var closeSquareBrackets:* = null;
      ...
      private function onMouseMove(pEvent:MouseEvent) : void
      {
         ...
         if(this._tText.length > 0)
         {
            ...
            for(i = 0; i < numLines; )
            {
               textHeight = Number(textHeight + lineHeight);
