package com.ankamagames.berilia.components
{
   ...
   public class Slot extends ButtonContainer implements ISlotDataHolder, FinalizableUIComponent, IDragAndDropHandler
   {
      ...
      public function refresh() : void
      {
         ...
         this._icon.transform.colorTransform = !!iconGreyedOut?GraphicContainer.COLOR_TRANSFORM_DISABLED:!!this._customColorTransform?this._customColorTransform:GraphicContainer.COLOR_TRANSFORM_DEFAULT;
         this._icon.greyedOut = iconGreyedOut;
         if(this._data is ICustomSlotData)
         {
            ...
            if(customSlotData.colorTransform)
            {
               if(iconGreyedOut)
               {
                  this._icon.transform.colorTransform = ColorUtils.mixColorTransforms(customSlotData.colorTransform,GraphicContainer.COLOR_TRANSFORM_DISABLED);
               }
               else
               {
                  this._icon.transform.colorTransform = customSlotData.colorTransform;
               }
            }
            if(customSlotData.size.x >= customSlotData.size.y && customSlotData.size.x >= width)
            {
               this._icon.width = width;
               this._icon.height = width * (customSlotData.size.y / customSlotData.size.x);
               this._icon.x = 0;
               this._icon.y = height - this._icon.height;
            }
            else if(customSlotData.size.y >= customSlotData.size.x && customSlotData.size.y >= height)
            {
               this._icon.width = height * (customSlotData.size.x / customSlotData.size.y);
            ...
            {
               this._icon.width = customSlotData.size.x;
               this._icon.height = customSlotData.size.y;
               this._icon.x = (width - this._icon.width) / 2;
               this._icon.y = height - this._icon.height;
            }
         }
         ...
         switch(true)
         {
            ...
            case msg is MouseReleaseOutsideMessage:
               ...
               if(linkCursor && this._dragging && !(dropTarget is ISlotDataHolder))
               {
                  ...
                  if(linkCursor != null)
                  {
                     KernelEventsManager.getInstance().processCallback(BeriliaHookList.DropEnd,SlotDragAndDropData(linkCursor.data).currentHolder);
                  }
               }
            ...
            case msg is MouseUpMessage:
               ...
               if(linkCursor && linkCursor.data is SlotDragAndDropData)
               {
                  ...
                  if(this._allowDrag)
                  {
                     KernelEventsManager.getInstance().processCallback(BeriliaHookList.DropEnd,dragData.currentHolder);
                  }
               }
               if(this._dragging)
               {
