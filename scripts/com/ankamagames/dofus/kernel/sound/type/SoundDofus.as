package com.ankamagames.dofus.kernel.sound.type
{
   ...
   public class SoundDofus implements ISound, ILocalizedSound
   {
      ...
      public function play(pLoop:Boolean = false, pLoops:int = 0, pFadeIn:VolumeFadeEffect = null, pFadeOut:VolumeFadeEffect = null) : void
      {
         ...
         if(pFadeIn)
         {
            fadeInBeg = Number(pFadeIn.beginningValue);
            fadeInEnd = Number(pFadeIn.endingValue);
            fadeInTime = Number(pFadeIn.timeFade);
         }
         if(pFadeOut)
         {
            fadeOutBeg = Number(pFadeOut.beginningValue);
            fadeOutEnd = Number(pFadeOut.endingValue);
            fadeOutTime = Number(pFadeOut.timeFade);
         }
      ...
      public function stop(pFadeOut:VolumeFadeEffect = null, mustStopCompletly:Boolean = false) : void
      {
         ...
         if(pFadeOut != null)
         {
            fadeOutB = Number(pFadeOut.beginningValue);
            fadeOutE = Number(pFadeOut.endingValue);
            fadeOutT = Number(pFadeOut.timeFade);
         }
