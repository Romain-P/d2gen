package com.ankamagames.dofus.kernel.sound.manager
{
   ...
   public class ClassicSoundManager extends EventDispatcher implements ISoundManager
   {
      ...
      public function setSubArea(pMap:Map = null) : void
      {
         ...
         §§push(Boolean(this.soundIsActivate));
         if(this.soundIsActivate)
         {
            this._localizedSoundsManager.playLocalizedSounds();
         }
         ...
         {
            if(plV[0] == PlaylistTypeEnum.ROLEPLAY_MUSIC)
            {
               musicPlaylist = Playlist.getPlaylistById(plV[1]);
            }
            else if(plV[0] == PlaylistTypeEnum.AMBIANCE_MUSIC)
            {
               ambiantPlaylist = Playlist.getPlaylistById(plV[1]);
            }
         }
         this._ambientManager.setAmbientSounds(subArea.ambientSounds,mp.sounds,musicPlaylist,ambiantPlaylist);
      ...
      public function handleFLAEvent(pAnimationName:String, pType:String, pParams:String, pSprite:Object = null) : void
      {
         ...
         var entityId:Number = pSprite.id;
         ...
         {
            soundType = uint(EnumSoundType.LOCALIZED_SOUND);
         }
         else
         {
            soundType = uint(EnumSoundType.UNLOCALIZED_SOUND);
         }
         busId = uint(SoundUtil.getBusIdBySoundId(soundId));
      ...
      public function playStopableSound(pSoundId:String) : void
      {
         var busId:* = 0;
         var soundPath:* = null;
         var soundUri:* = null;
         ...
         if(this._forceSounds)
         {
            busId = uint(SoundUtil.getBusIdBySoundId(pSoundId));
            soundPath = SoundUtil.getConfigEntryByBusId(busId);
            soundUri = new Uri(soundPath + pSoundId + ".mp3");
            isound = SoundFactory.getSound(EnumSoundType.UNLOCALIZED_SOUND,soundUri);
            isound.busId = busId;
            isound.volume = 1;
            this._stopableSounds[pSoundId] = isound;
            isound.play();
         }
         else
         {
            KernelEventsManager.getInstance().processCallback(HookList.StopableSoundEndend,pSoundId);
         }
      }
