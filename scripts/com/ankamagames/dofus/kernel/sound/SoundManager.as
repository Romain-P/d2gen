   ...
   public class SoundManager
   {
      ...
      public function setSoundOptions() : void
      {
         ...
         try
         {
            ...
            this._globalVolume = OptionManager.getOptionManager("tubul")["globalVolume"];
            this.setMusicVolume(!!musicMute?0:Number(OptionManager.getOptionManager("tubul")["volumeMusic"]));
            this.setSoundVolume(!!soundMute?0:Number(OptionManager.getOptionManager("tubul")["volumeSound"]));
            this.setAmbienceVolume(!!ambientSoundMute?0:Number(OptionManager.getOptionManager("tubul")["volumeAmbientSound"]));
            RegConnectionManager.getInstance().send(ProtocolEnum.OPTION_MUSIC_LOOP_VALUE_CHANGED,infiniteLoop);
            soundIsDeactivated = OptionManager.getOptionManager("tubul")["tubulIsDesactivated"];
      ...
      private function onPropertyChanged(e:PropertyChangeEvent) : void
      {
         ...
         {
            case "muteMusic":
               this.setMusicVolume(!!e.propertyValue?0:Number(this._tuOptions.volumeMusic));
               break;
            case "muteSound":
               this.setSoundVolume(!!e.propertyValue?0:Number(this._tuOptions.volumeSound));
               break;
            case "muteAmbientSound":
               this.setAmbienceVolume(!!e.propertyValue?0:Number(this._tuOptions.volumeAmbientSound));
               break;
            case "volumeMusic":
            ...
            case "globalVolume":
               ...
               if(this._tuOptions.muteMusic == false)
               {
                  this.setMusicVolume(this._tuOptions.volumeMusic);
               }
               if(this._tuOptions.muteSound == false)
               {
                  this.setSoundVolume(this._tuOptions.volumeSound);
               }
               if(this._tuOptions.muteAmbientSound == false)
               {
                  this.setAmbienceVolume(this._tuOptions.volumeAmbientSound);
                  break;
               }
               break;
