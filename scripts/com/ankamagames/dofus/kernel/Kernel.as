package com.ankamagames.dofus.kernel
{
   ...
   import com.ankamagames.dofus.logic.game.common.managers.TimedNotificationManager;
   ...
   public class Kernel
   {
      ...
      public function init(stage:Stage, rootClip:DisplayObject) : void
      {
         ...
         if(UpdaterConnexionHelper.hasUpdaterOrZaapArguments())
         {
            UpdaterApi.init();
         }
      }
      ...
      public function postInit() : void
      {
         ...
         TimedNotificationManager.getInstance();
      ...
      public function reset(messagesToDispatchAfter:Array = null, autoRetry:Boolean = false, reloadData:Boolean = false) : void
      {
         var msg:* = null;
         TooltipManager.hide();
         if(SteamManager.hasSteamApi() && SteamManager.getInstance().isSteamEmbed() || UpdaterApi.isUsingZaapLogin())
         {
      ...
      public function initOptions() : void
      {
         ...
         ao.pngPathOverride = LangManager.getInstance().getEntry("config.gfx.path.world.pngOverride");
         ao.swfPath = LangManager.getInstance().getEntry("config.gfx.path.world.swf");
         ...
         ao.tacticalModeTemplatesPath = LangManager.getInstance().getEntry("config.atouin.path.tacticalModeTemplates");
         ...
         if(ao.pngPathOverride.charAt(0) == "!")
         {
            ao.pngPathOverride = "";
         }
