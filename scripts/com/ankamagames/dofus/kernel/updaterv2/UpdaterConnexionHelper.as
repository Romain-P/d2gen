package com.ankamagames.dofus.kernel.updaterv2
{
   ...
   public class UpdaterConnexionHelper
   {
      ...
      {
         if(this._zaap && this._zaap.client != null)
         {
            return true;
         }
         if(this._socket && this._socket.connected)
      ...
      private function onConnectionOpened(event:Event = null) : void
      {
         if(this._zaap && this._zaap.client != null)
         {
            _log.info("Connected to Zaap on port : " + CommandLineArguments.getInstance().getArgument("port"));
         }
         else
         {
         ...
         StatisticReportingManager.getInstance().report((this._zaap && this._zaap.client != null?"ZaapConnection":"UpdaterConnexion") + " - " + BuildInfos.BUILD_TYPE + " - " + BuildInfos.VERSION,"success");
