package com.ankamagames.dofus.network.messages.game.approach
{
   ...
   public class AccountCapabilitiesMessage extends NetworkMessage implements INetworkMessage
   {
      ...
      public function initAccountCapabilitiesMessage(accountId:uint = 0, tutorialAvailable:Boolean = false, breedsVisible:uint = 0, breedsAvailable:uint = 0, status:int = -1, canCreateNewCharacter:Boolean = false, unlimitedRestatEndDate:Number = 0) : AccountCapabilitiesMessage
      {
      ...
      {
         var _box0:* = 0;
         _box0 = uint(BooleanByteWrapper.setFlag(_box0,0,this.tutorialAvailable));
         _box0 = uint(BooleanByteWrapper.setFlag(_box0,1,this.canCreateNewCharacter));
      ...
      private function _unlimitedRestatEndDateFunc(input:ICustomDataInput) : void
      {
         ...
         if(this.unlimitedRestatEndDate < 0 || this.unlimitedRestatEndDate > 9007199254740990)
         {
            throw new Error("Forbidden value (" + this.unlimitedRestatEndDate + ") on element of AccountCapabilitiesMessage.unlimitedRestatEndDate.");
         }
      }
   }
}
