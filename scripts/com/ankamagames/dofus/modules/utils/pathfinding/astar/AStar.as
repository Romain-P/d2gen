package com.ankamagames.dofus.modules.utils.pathfinding.astar
{
   ...
   import com.ankamagames.dofus.modules.utils.pathfinding.world.Edge#156;
   ...
   public class AStar
   {
      ...
      private static var _forbiddenBecauseConquestAndNoPrismAllowedMapIds:Vector.<Number>;
      ...
      {
         var conquestSubareaId:int = 0;
         var conquestSubarea:* = null;
         var mapId:Number = NaN;
         _forbiddenSubareaIds = GameDataQuery.queryEquals(SubArea,"mountAutoTripAllowed",false);
         var conquestSubareaIds:Vector.<uint> = GameDataQuery.queryEquals(SubArea,"isConquestVillage",true);
         var notAllowingPrismMapIds:Vector.<uint> = GameDataQuery.queryEquals(MapPosition,"allowPrism",false);
         _forbiddenBecauseConquestAndNoPrismAllowedMapIds = new Vector.<Number>();
         for each(conquestSubareaId in conquestSubareaIds)
         {
            conquestSubarea = SubArea.getSubAreaById(conquestSubareaId);
            for each(mapId in conquestSubarea.mapIds)
            {
               if(notAllowingPrismMapIds.indexOf(mapId) != -1)
               {
                  _forbiddenBecauseConquestAndNoPrismAllowedMapIds.push(mapId);
               }
            }
         }
      }
      ...
      private static function compute(e:Event) : void
      {
         ...
         var node:* = null;
         for(var start:int = getTimer(); openList.length > 0; )
         {
      ...
      private static function hasValidTransition(edge:Edge#156) : Boolean
      {
         var criterion:* = null;
         var transition:* = null;
         var criterionWhiteList:* = ["Ad","DM","MI","Mk","Oc","Pc","QF","Qo","Qs","Sv"];
      ...
      private static function hasValidDestinationSubarea(edge:Edge#156) : Boolean
      {
         ...
         if(_forbiddenSubareaIds.indexOf(toSubareaId) != -1 || _forbiddenBecauseConquestAndNoPrismAllowedMapIds.indexOf(toMapId) != -1)
         {
            return false;
         }
         return true;
      ...
      private static function callbackWithResult(result:Vector.<Edge#156>) : void
      {
      ...
      private static function buildResultPath(worldGraph:WorldGraph, node:Node) : Vector.<Edge#156>
      {
         for(var result:Vector.<Edge> = new Vector.<Edge#156>(); node.parent != null; )
         {
