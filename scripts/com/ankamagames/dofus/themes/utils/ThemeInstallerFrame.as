      ...
      private function processJsonList(jsonArray:*) : void
      {
         ...
         for each(themeEntry in jsonArray)
         {
            ...
            if(theme)
            {
               ...
               for(entryVersionArray = String(themeEntry.version).split("."); theme.version.length > entryVersionArray.length; )
               {
                  entryVersionArray.push(0);
               }
               for(i = 0; i < theme.version.length; )
               {
      ...
      private function processZipEntryForUpdate() : void
      {
         ...
         while(this._fileToInstall.length && getTimer() - startProcess < 16)
         {
            ...
            if(!unzipedFile.exists)
            {
               ...
               {
                  this.writeZipEntry(null,entry,null,unzipedFile,this._currentZipFile);
               }
            }
            else if(!entry.isDirectory())
            {
               ...
               fileCrcValue = uint(this.fileCrc.getValue());
      ...
      private function finishUpdate() : void
      {
         ...
         if(this._totalTreatedFile == this._currentZipFile.entries.length)
         {
            ...
            if(theme)
            {
               ...
               for(version = version.length > 3?version.slice(0,3):version; version.length < 3; )
               {
                  version.push(0);
               }
