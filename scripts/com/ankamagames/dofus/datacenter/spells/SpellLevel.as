   ...
   public class SpellLevel implements ICellZoneProvider, IDataCenter
   {
      ...
      public function get spellZoneEffects() : Vector.<IZoneShape>
      {
         ...
         if(!this._spellZoneEffects)
         {
            ...
            numEffects = uint(this.effects.length);
      ...
      {
         for(var i:int = 0; i < this.spellZoneEffects.length; )
         {
      ...
      public function get canSummon() : Boolean
      {
         ...
         {
            if(effect.effectId == ActionIdEnum.ACTION_CHARACTER_ADD_DOUBLE || effect.effectId == ActionIdEnum.ACTION_SUMMON_SLAVE)
            {
               return true;
            }
            if(effect.effectId == ActionIdEnum.ACTION_SUMMON_CREATURE || effect.effectId == ActionIdEnum.ACTION_KILL_AND_SUMMON_CREATURE || effect.effectId == ActionIdEnum.ACTION_KILL_AND_SUMMON_SLAVE)
            {
      ...
      public function get canBomb() : Boolean
      {
         ...
         for each(effect in this.effects)
         {
            if(effect.effectId == ActionIdEnum.ACTION_SUMMON_BOMB)
            {
               return true;
            }
            if(effect.effectId == ActionIdEnum.ACTION_SUMMON_CREATURE || effect.effectId == ActionIdEnum.ACTION_KILL_AND_SUMMON_CREATURE || effect.effectId == ActionIdEnum.ACTION_KILL_AND_SUMMON_SLAVE)
      ...
      public function get canThrowPlayer() : Boolean
      {
         ...
         for each(effect in this.effects)
         {
            if(effect.effectId == ActionIdEnum.ACTION_THROW_CARRIED_CHARACTER)
            {
               return true;
            }
         }
