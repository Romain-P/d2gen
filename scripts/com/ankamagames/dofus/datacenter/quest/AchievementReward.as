package com.ankamagames.dofus.datacenter.quest
{
   ...
   public class AchievementReward implements IDataCenter
   {
      ...
      public function toString() : String
      {
         ...
         {
            text = text + ("     kamasRatio " + this.kamasRatio + "   (scale with player ? " + this.kamasScaleWithPlayerLevel + ")");
         }
         if(this.experienceRatio > 0)
         {
            text = text + ("     experienceRatio " + this.experienceRatio);
         }
         if(this.itemsReward.length > 0)
         {
            text = text + ("     itemsReward " + this.itemsReward);
         }
         if(this.emotesReward.length > 0)
         {
            text = text + ("     emotesReward " + this.emotesReward);
         }
         if(this.spellsReward.length > 0)
         {
            text = text + ("     spellsReward " + this.spellsReward);
         }
         if(this.titlesReward.length > 0)
         {
            text = text + ("     titlesReward " + this.titlesReward);
         }
         if(this.ornamentsReward.length > 0)
         {
            text = text + ("     ornamentsReward " + this.ornamentsReward);
         }
         return text;
      }
   }
