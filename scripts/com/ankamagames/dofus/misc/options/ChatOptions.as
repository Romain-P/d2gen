package com.ankamagames.dofus.misc.options
{
   ...
   public dynamic class ChatOptions extends OptionManager
   {
      ...
      private function onCssLoaded(cssUrl:String) : void
      {
         ...
         var theme:String = (OptionManager.getOptionManager("dofus") as DofusOptions)["currentUiSkin"];
         var chatTheme:String = chatOptions.currentChatTheme;
         var channelsCount:int = ChatChannel.getChannels().length;
         for(var i:int = 0; i < channelsCount; )
         {
            ...
            if(theme != chatTheme)
            {
               chatOptions["channelColor" + i] = this.colors[i];
            }
            i++;
         }
         if(theme != chatTheme)
         {
            chatOptions.currentChatTheme = theme;
         }
