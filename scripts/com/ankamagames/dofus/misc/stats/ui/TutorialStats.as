package com.ankamagames.dofus.misc.stats.ui
{
   ...
   public class TutorialStats implements IUiStats, IHookStats
   {
         ...
         switch(true)
         {
            ...
            case pMessage is QuestValidatedMessage:
               ...
               if(qvmsg.questId == START_QUEST_ID && this._currentStep >= TUTORIAL_STEP_FIGHT_WIN_2 && this._currentStep < TUTORIAL_STEP_END_BASE_TUTORIAL)
               {
                  ...
                  {
                     SteamManager.getInstance().sendRedShellEvent("tutorial_completed");
                     break;
                  }
                  break;
               }
      ...
      public function onHook(pHook:Hook, pArgs:Array) : void
      {
         ...
         switch(pHook.name)
         {
            case QuestHookList.QuestStepValidated.name:
               ...
               {
                  stepId = uint(this.getStepEventId(pArgs[1]));
            ...
            case HookList.TutorialStep.name:
               if(pArgs[0] > 1)
               {
                  step = uint(this.getStepEventId(this._quest.steps[pArgs[0] - 2].id));
      ...
      private function getStepEventId(pQuestStep:uint) : uint
      {
         ...
         {
            case 1042:
               stepId = uint(TUTORIAL_STEP_PLAYER_MOVE);
               break;
            case 1043:
               stepId = uint(TUTORIAL_STEP_TALK_TO_NPC);
               break;
            case 1044:
               stepId = uint(TUTORIAL_STEP_CLOSE_INVENTORY);
               break;
            case 1045:
               stepId = uint(TUTORIAL_STEP_CHANGE_MAP);
               break;
            case 1046:
               stepId = uint(TUTORIAL_STEP_START_FIGHT);
               break;
            case 1047:
               stepId = uint(TUTORIAL_STEP_FIGHT_START_POSITION);
               break;
            case 1048:
               stepId = uint(TUTORIAL_STEP_FIGHT_MOVE);
               break;
            case 1049:
               stepId = uint(TUTORIAL_STEP_FIGHT_USE_SPELL);
               break;
            case 1050:
               stepId = uint(TUTORIAL_STEP_FIGHT_END_TURN);
               break;
            case 1051:
               stepId = uint(TUTORIAL_STEP_FIGHT_WIN);
               break;
            case 1052:
               stepId = uint(TUTORIAL_STEP_START_QUEST);
               break;
            case 1053:
               stepId = uint(TUTORIAL_STEP_START_FIGHT_2);
               break;
            case 1059:
               stepId = uint(TUTORIAL_STEP_END_BASE_TUTORIAL);
               break;
            case 1060:
               stepId = uint(TUTORIAL_STEP_EQUIP_ALL_ITEMS);
               break;
            case 1061:
               stepId = uint(TUTORIAL_STEP_FIGHT_WIN_2);
         }
         return stepId;
      }
