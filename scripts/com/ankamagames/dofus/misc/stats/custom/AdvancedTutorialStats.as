package com.ankamagames.dofus.misc.stats.custom
{
   ...
   public class AdvancedTutorialStats implements IStatsClass, IHookStats
   {
      ...
      private static const _log:Logger = Log.getLogger(getQualifiedClassName(AdvancedTutorialStats));
      ...
      private static const IDOL_ID:uint = 31;
      ...
      {
         this._jobStepsHarvest = new Array();
      ...
      public function process(pMessage:Message, pArgs:Array = null) : void
      {
         ...
         {
            target = pArgs[1];
            if(target.getUi().name == "rewardsAndGiftsUi" && target.name == "btn_acceptAll")
            {
         ...
         else if(pMessage is MapComplementaryInformationsDataMessage)
         {
            ...
            switch(mcidmsg.mapId)
            {
               ...
               case HUB_MAP_ID:
                  ...
                  if(this._currentStep == TUTORIAL_STEP_TALK_TO_DARM_2)
                  {
                     this.sendStepValidation(TUTORIAL_STEP_CHANGE_MAP_6);
                     break;
                  }
                  if(this._activeQuestId == QUEST_JOB_ID)
                  {
                     this._currentStep = TUTORIAL_STEP_CHANGE_MAP_4;
                     break;
                  }
                  if(this._activeQuestId == QUEST_FIGHT_ID)
                  {
      ...
      public function onHook(pHook:Hook, pArgs:Array) : void
      {
         ...
         switch(pHook.name)
         {
            case QuestHookList.QuestObjectiveValidated.name:
               if(pArgs[0] == QUEST_JOB_ID || pArgs[0] == QUEST_FIGHT_ID)
               {
                  if(this._currentStep == TUTORIAL_STEP_START_END_NPC_DIALOG)
                  {
                     ...
                     {
                        stepId = uint(TUTORIAL_STEP_END_JOB_QUEST);
                     }
                  }
                  else
                  {
                     stepId = uint(this.getQuestStepEventId(pArgs[1]));
                  }
               ...
               break;
            case BeriliaHookList.UiLoaded.name:
               if(pArgs[0] == "rewardsAndGiftsUi")
               {
                  if(this._currentStep < TUTORIAL_STEP_CRAFT_IDOL)
                  {
            ...
            case HookList.TutorialStep.name:
               stepId = uint(this.getStepFromQuestInfos(this._currentQuest));
               if(stepId > this._currentStep)
               {
            ...
            case BeriliaHookList.MouseClick.name:
                  ...
                  if(entitiesFrame)
                  {
                     ...
                     if(infos)
                     {
                        if(infos.npcId == 2897)
                        {
                           ...
                           if(this._currentStep > TUTORIAL_STEP_CLICK_ON_JORIS_4 && this._currentStep < TUTORIAL_STEP_CLICK_ON_JORIS_5)
                           {
                              this.sendStepValidation(TUTORIAL_STEP_CLICK_ON_JORIS_5);
                              break;
                           }
                           if(this._currentStep > TUTORIAL_STEP_CLICK_ON_JORIS_5 && this._currentStep < TUTORIAL_STEP_CLICK_ON_JORIS_6)
                           {
                        ...
                        if(infos.npcId == 2895)
                        {
                           ...
                           if(this._currentStep < TUTORIAL_STEP_CLICK_ON_HOBOULO_2)
                           {
                        ...
                        if(infos.npcId == 2896)
                        {
                           ...
                           if(this._currentStep < TUTORIAL_STEP_CLICK_ON_DARM_2)
                           {
               ...
               break;
            case QuestHookList.QuestStarted.name:
            ...
            case QuestHookList.QuestInfosUpdated.name:
               ...
               if(pArgs[0] == QUEST_JOB_ID || pArgs[0] == QUEST_FIGHT_ID)
               {
                  ...
                  if(this._currentQuest.questId == QUEST_JOB_ID)
                  {
                     ...
                     if(this._jobStepsHarvest.length)
                     {
               ...
               break;
            case HookList.DisplayUiArrow.name:
      ...
      private function getQuestStepEventId(pQuestStep:uint) : uint
      {
         ...
         {
            case 9655:
               stepId = uint(TUTORIAL_STEP_CHANGE_MAP_3);
               break;
            case 9656:
               stepId = uint(TUTORIAL_STEP_START_END_NPC_DIALOG);
               break;
            case 9657:
            case 9658:
            case 9659:
            ...
            case 9661:
               break;
            case 9662:
               stepId = uint(TUTORIAL_STEP_CRAFT_IDOL);
               break;
            case 10015:
               stepId = uint(TUTORIAL_STEP_END_CRAFT_TUTORIAL);
               break;
            case 9663:
               stepId = uint(TUTORIAL_STEP_CHANGE_MAP_4);
               break;
            case 9680:
               stepId = uint(TUTORIAL_STEP_CHANGE_MAP_5);
               break;
            case 9685:
               stepId = uint(TUTORIAL_STEP_TALK_TO_DARM);
               break;
            case 9720:
               stepId = uint(TUTORIAL_STEP_FIGHT_WIN);
               break;
            case 10121:
               stepId = uint(TUTORIAL_STEP_FIGHT_WIN_BIS);
               break;
            case 10016:
               stepId = uint(TUTORIAL_STEP_TALK_TO_DARM_2);
               break;
            case 9734:
               stepId = uint(TUTORIAL_STEP_CHANGE_MAP_6);
         }
         return stepId;
      }
      ...
      private function getStepFromQuestInfos(pQuestInfos:Object) : uint
      {
         ...
         if(pQuestInfos)
         {
            ...
            for(i = 0; i < step.objectives.length; )
            {
               if(pQuestInfos.objectives[step.objectives[i].id] == false)
               {
                  stepId = uint(step.objectives[i].id);
               }
               i++;
            }
            if(stepId > 0)
            {
               stepId = uint(this.getQuestStepEventId(stepId));
            }
         }
         return stepId;
      }
