package com.ankamagames.dofus.misc.utils
{
   ...
   public class Inspector
   {
      ...
      private var _lastTarget:Vector.<InteractiveItem#502>;
      
      private var _currentShortCut:Vector.<ShortcutItem#502>;
      ...
      private function onMouseMove(e:MouseEvent) : void
      {
         ...
         var ind:String = "";
         for(var i:uint = 0; i < this._lastTarget.length; )
         {
         ...
         if(this._lastTarget.length)
         {
            ...
            this._currentShortCut = !!this._lastTarget[0].shortcuts?this._lastTarget[0].shortcuts.concat():new Vector.<ShortcutItem#502>();
            this._currentShortCut.unshift(new ShortcutItem#502("Copier toutes les informations",Keyboard.C,function():void
            {
               var tf:* = new TextField();
               tf.htmlText = rawTooltipStr.split("<br/>").join("\n");
               Clipboard.generalClipboard.setData(ClipboardFormats.TEXT_FORMAT,tf.text);
            },true,false,false));
            this._currentShortCut.unshift(new ShortcutItem#502("Activer l\'inspection avancée",Keyboard.Z,function():void
            {
      ...
      private function findElements(target:DisplayObject) : Vector.<InteractiveItem#502>
      {
         var currentItem:* = null;
         var result:Vector.<InteractiveItem> = new Vector.<InteractiveItem#502>();
         var ind:String = "";
      ...
      private function findSameLevelElements() : Vector.<InteractiveItem#502>
      {
         var currentItem:* = null;
         var item:* = null;
         var result:Vector.<InteractiveItem> = new Vector.<InteractiveItem#502>();
      ...
      private function cast(target:DisplayObject) : InteractiveItem#502
      {
         var currentItem:* = null;
         switch(true)
         {
            ...
            case target is GraphicContainer:
               currentItem = new InteractiveItemUi#502();
      ...
      {
         var current:* = null;
         for(var i:int = 0; i < target.numChildren; )
         {
...
class ShortcutItem#502
{
   ...
   function ShortcutItem#502(legend:String, key:uint, callback:Function, ctrl:Boolean, shift:Boolean, alt:Boolean)
   {
      super();
      this.legend = legend;
      this.key = key;
...
class InteractiveItem#502
{
   ...
   public var shortcuts:Vector.<ShortcutItem#502>;
   
   public var target:DisplayObject;
   
   function InteractiveItem#502()
   {
      super();
   }
   
   public function highlight(index:uint, elements:Vector.<InteractiveItem#502>) : void
   {
   }
...
class InteractiveItemUi#502 extends InteractiveItem#502
{
    
   
   function InteractiveItemUi#502()
   {
      super();
      shortcuts = Vector.<ShortcutItem#502>([new ShortcutItem#502("Cmd click test fonctionnel",Keyboard.Q,function():void
      {
         toClipboard("click( UiHelper.get(\"" + GraphicContainer(target).getUi().uiData.name + "\"), \"" + GraphicContainer(target).customUnicName + "\");");
      },true,true,false),new ShortcutItem#502("Cmd son survol",Keyboard.S,function():void
      {
      ...
      },true,true,false),new ShortcutItem#502("Cmd son click",Keyboard.X,function():void
      {
         toClipboard("/adduisoundelement " + GraphicContainer(target).getUi().uiData.name + " " + target.name + " onRelease [ID_SON]");
      },true,true,false),new ShortcutItem#502("Cmd inspecter element",Keyboard.A,function():void
      {
         toClipboard("/inspectuielement " + GraphicContainer(target).getUi().uiData.name + " " + target.name);
      },true,true,false)]);
   }
   
   override public function highlight(index:uint, elements:Vector.<InteractiveItem#502>) : void
   {
      var pos:* = null;
      var elem:* = null;
      ...
      for each(elem in elements)
      {
         ...
         if(elem is InteractiveItemUi#502 && elem.target != target)
         {
            firstUi = false;
            break;
         }
   ...
   override public function tooltip() : String
   {
      ...
      else
      {
         ...
         str = str + ("<b><font color=\'#964D8C\'>Composant </font><font color=\'" + this.hashStringToColor(cptClass) + "\' >" + cptClass + "</font></b><br/>");
         str = str + ("<b>Nom : </b><font color=\'" + color + "\'>" + target.name + "</font><br/>");
         switch(true)
         {
            case target is TextureBase:
               str = str + ("<b>Chemin : </b>" + TextureBase(target).uri + "<br/>");
               break;
            case target is Label:
               str = str + ("<b>Css : </b>" + Label(target).css + "<br/>");
               str = str + ("<b>Classe css : </b>" + Label(target).cssClass + "<br/>");
               break;
            case target is Slot:
               str = str + ("<b>emptyTexture : </b>" + Slot(target).emptyTexture + "<br/>");
               str = str + ("<b>acceptDragTexture : </b>" + Slot(target).acceptDragTexture + "<br/>");
               str = str + ("<b>refuseDragTexture : </b>" + Slot(target).refuseDragTexture + "<br/>");
         }
...
class InteractiveItemCell extends InteractiveItem#502
{
...
class InteractiveItemEntity extends InteractiveItem#502
{
      ...
      if(info)
      {
         switch(true)
         {
            case info is GameRolePlayNpcInformations:
               ...
               shortcuts.push(new ShortcutItem#502("Ouvrir l\'admin lite de ce PNJ",Keyboard.A,function():void
               {
...
class InteractiveItemElement extends InteractiveItem#502
{
...
class InteractiveItemMapBorder extends InteractiveItem#502
{
   ...
   override public function tooltip() : String
   {
      ...
      switch(target)
      {
         ...
         case FrustumManager.getInstance().getShape(DirectionsEnum.RIGHT):
            prc = uint(Math.round(mb.mouseY / mb.height * 100));
            break;
         case FrustumManager.getInstance().getShape(DirectionsEnum.UP):
         case FrustumManager.getInstance().getShape(DirectionsEnum.DOWN):
            prc = uint(Math.round(mb.mouseX / mb.width * 100));
      }
      var str:* = "<b><font color=\'#40BD00\'>Bord de map " + DescribeTypeCache.getConstantName(DirectionsEnum,mb.direction) + " (" + prc + "%)</font></b>";
      return str;
   }
}
