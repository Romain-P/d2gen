package com.ankamagames.dofus.misc.utils
{
   ...
   public class GameDataQuery
   {
      ...
      public static function union(... idsVectors) : Vector.<uint>
      {
         ...
         for(var i:int = 0; i < idsVectors.length; i++)
         {
            if(idsVectors[i] != null)
            {
               for(ind = 0; ind < idsVectors[i].length; )
               {
                  id = uint(idsVectors[i][ind]);
                  if(!added[id])
                  {
                     result.push(id);
                     added[id] = true;
      ...
      private static function getSortFunction(datas:*, fieldNames:*, ascending:*) : Function
      {
         ...
         {
            for(var fieldIndex:int = 0; fieldIndex < maxFieldIndex; )
            {
      ...
      private static function checkPackage(target:Class) : Class
      {
         ...
         if(packageName == "d2data")
         {
            ...
            for each(constant in desc..constant)
            {
               ...
               {
                  return GameDataField.getClassByName(constant.@type.toString());
               }
            }
         }
