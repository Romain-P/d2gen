package com.ankamagames.dofus.logic.connection.frames
{
   ...
   public class InitializationFrame implements Frame, IUpdaterMessageHandler
   {
      ...
      private function initAfterLoadConfig() : void
      {
         ...
         var fontLocation:String = LangManager.getInstance().getEntry("config.ui.asset.fontsList");
         this._aFiles.push(LangManager.getInstance().getEntry("config.ui.asset.fontsList"));
         for(var i:int = 0; i < this._aFiles.length; i++)
         {
      ...
      private function checkInit() : void
      {
         ...
         var key:* = null;
         var d:* = null;
         var testSequence:* = null;
         var type:* = null;
         var i:int = 0;
         var lowdefMappings:* = null;
         var skin:* = null;
         var dataClassDesc:* = null;
         var fct:* = null;
      ...
      private function setModulePercent(moduleName:String, prc:Number, add:Boolean = false) : void
      {
         ...
         {
            totalPrc = Number(totalPrc + p / 100 * this._percentPerModule);
         }
         id = uint(Dofus.getInstance().instanceId);
