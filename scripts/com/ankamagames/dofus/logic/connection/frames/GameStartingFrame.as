package com.ankamagames.dofus.logic.connection.frames
{
   ...
   public class GameStartingFrame implements Frame
   {
      ...
      public function pushed() : Boolean
      {
         ...
         Dofus.getInstance().renameApp("Dofus " + BuildInfos.VERSION.toString());
         return true;
      }
      ...
      private function systemMessageDisplay(msg:SystemMessageDisplayMessage) : void
      {
         ...
         {
            textId = uint(InfoMessage.getInfoMessageById(40000 + msg.msgId).textId);
         }
         else
         {
            _log.error("Information message " + (40000 + msg.msgId) + " cannot be found.");
            textId = uint(InfoMessage.getInfoMessageById(207).textId);
            a = new Array();
            a.push(msg.msgId);
         }
