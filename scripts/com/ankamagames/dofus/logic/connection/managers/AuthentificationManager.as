package com.ankamagames.dofus.logic.connection.managers
{
   ...
   public class AuthentificationManager implements IDestroyable
   {
      ...
      public function setSalt(salt:String) : void
      {
         ...
         {
            for(_log.warn("Authentification salt size is lower than 32 "); this._salt.length < 32; )
            {
      ...
      public function getIdentificationMessage() : IdentificationMessage
      {
         ...
         if(this._lva.username.indexOf("|") == -1)
         {
            ...
            imsg.version.initVersionExtended(BuildInfos.VERSION.major,BuildInfos.VERSION.minor,BuildInfos.VERSION.release,BuildInfos.VERSION.revision,BuildInfos.VERSION.patch,buildType,ClientInstallTypeEnum.CLIENT_BUNDLE,ClientTechnologyEnum.CLIENT_AIR);
            return imsg;
         }
         this.ankamaPortalKey = this.cipherMd5String(this._lva.password);
         ...
         iafmsg.version.initVersionExtended(BuildInfos.VERSION.major,BuildInfos.VERSION.minor,BuildInfos.VERSION.release,BuildInfos.VERSION.revision,BuildInfos.VERSION.patch,buildType,ClientInstallTypeEnum.CLIENT_BUNDLE,ClientTechnologyEnum.CLIENT_AIR);
         return iafmsg;
      }
