package com.ankamagames.dofus.logic.common.managers
{
   ...
   public class AbstractItemFilterManager
   {
      ...
      {
         var id:int = 0;
         var effectsId:Array = [ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_LOST,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_LOST_FROM_WATER,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_LOST_FROM_EARTH,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_LOST_FROM_AIR,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_LOST_FROM_FIRE,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_STEAL_FROM_WATER,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_STEAL_FROM_EARTH,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_STEAL_FROM_AIR,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_STEAL_FROM_FIRE,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_STEAL,ActionIdEnum.ACTION_CHARACTER_ACTION_POINTS_LOST,ActionIdEnum.ACTION_CHARACTER_LIFE_POINTS_WIN,ActionIdEnum.ACTION_HUNT_TOOL,ActionIdEnum.ACTION_INCARNATION];
         this._weaponEffects = [];
      ...
      {
         var id:int = 0;
         var effectsId:Array = [DataEnum.ITEM_TYPE_BOW,DataEnum.ITEM_TYPE_HAMMER,DataEnum.ITEM_TYPE_CROSSBOW,DataEnum.ITEM_TYPE_MAGIC_WEAPON,DataEnum.ITEM_TYPE_ROD,DataEnum.ITEM_TYPE_STAFF,DataEnum.ITEM_TYPE_DAGGER,DataEnum.ITEM_TYPE_SCYTHE,DataEnum.ITEM_TYPE_AXE,DataEnum.ITEM_TYPE_SHOVEL,DataEnum.ITEM_TYPE_PICKAXE,DataEnum.ITEM_TYPE_SWORD];
         this._targetWeapon = [];
         for each(id in effectsId)
         {
            this._targetWeapon.push(this._dataApi.getItemType(id));
         }
      }
      ...
      {
         var id:int = 0;
         var effectsId:Array = [ActionIdEnum.ACTION_CHARACTER_GAIN_WISDOM,ActionIdEnum.ACTION_CHARACTER_GAIN_STRENGTH,ActionIdEnum.ACTION_CHARACTER_GAIN_CHANCE,ActionIdEnum.ACTION_CHARACTER_GAIN_AGILITY,ActionIdEnum.ACTION_CHARACTER_GAIN_INTELLIGENCE,ActionIdEnum.ACTION_CHARACTER_GAIN_VITALITY,ActionIdEnum.ACTION_CHARACTER_BOOST_LIFE_POINTS,ActionIdEnum.ACTION_CHARACTER_ENERGY_POINTS_WIN];
         this._consumableEffects = [];
         for each(id in effectsId)
         {
      ...
      public function dataInit(specificFilters:Array = null) : void
      {
         ...
         {
            this._dataMatrix.push({
               "name":this._currentUi.uiClass.uiApi.getText("ui.common.categories"),
               "id":this._catNumber,
               "isCat":true
            });
            this._dataMatrix.push({
         ...
         this._catNumber++;
         this._dataMatrix.push({
            "name":this._currentUi.uiClass.uiApi.getText("ui.common.levels"),
            "id":this._catNumber,
            "isCat":true
         });
         this._dataMatrix.push({
      ...
      public function addSubCategory(superTypeId:Array, filterCatText:String = "", specificTypes:Vector.<uint> = null) : Array
      {
         ...
         this._currentUi.uiClass.sysApi.log(1,"add sub filters " + filterCatText + " before all Ids : " + (getTimer() - timer2));
         if(Vector.<uint>(subTypeIds).length > 0)
         {
            ...
            this._catNumber++;
            this._currentUi.uiClass.sysApi.log(1,"add sub filters " + filterCatText + " : " + (getTimer() - timer2));
      ...
      {
         var index:int = this._currentSubIds.indexOf(typeId);
         var itemType:ItemType = this._dataApi.getItemType(typeId);
         if(typeId > 0)
         {
            if(index == -1)
            {
               this._currentSubIds.push(typeId);
               if(this._subFilteredTypes.indexOf(itemType.superTypeId) == -1)
               {
                  this._subFilteredTypes.push(itemType.superTypeId);
               }
               if(this._currentFilteredIds.indexOf(itemType.superTypeId) == -1)
               {
                  this._currentFilteredIds.push(itemType.superTypeId);
               }
            }
            else
            {
               this._currentSubIds.splice(index,1);
               if(!this.checkSubFilters(itemType.superTypeId))
               {
                  this._subFilteredTypes.splice(this._subFilteredTypes.indexOf(itemType.superTypeId),1);
                  this._currentFilteredIds.splice(this._currentFilteredIds.indexOf(itemType.superTypeId,1));
               }
            }
         }
      }
      ...
      {
         var subId:int = 0;
         var tmpSuperTypeId:* = 0;
         for each(subId in this._currentSubIds)
         {
            tmpSuperTypeId = uint(this._dataApi.getItemType(subId).superTypeId);
      ...
      public function displayCategories(selectedCategory:Object = null) : Array
      {
         ...
         this._currentUi.uiClass.sysApi.setData("openedFilterCat",this._openedCategories);
      ...
      private function addLevelFilter() : Array
      {
         ...
         result = this.addEmptyLine(result,1);
         return result;
      }
      ...
      {
         var ids:* = null;
         var obtainingId:int = 0;
         var id:* = undefined;
         if(this._currentCategory == this.SKIN_CATEGORY)
         {
            ids = this._dataApi.queryIntersection(this.filterTargetEquipment(),this._dataApi.queryIntersection(this.filterType(),this.filterLevel()));
         }
         else
         {
            ids = this._dataApi.queryIntersection(this.filterCharac(),this._dataApi.queryIntersection(this.filterType(),this.filterLevel()));
         }
      ...
      private function filterType() : Vector.<uint>
      {
         var subId:int = 0;
         var id:int = 0;
         ...
         for each(subId in this._currentSubIds)
         {
            ...
            {
               typeResult = this._dataApi.queryIntersection(this._allItemsWithCurrentTypeIds,this._dataApi.queryUnion(typeResult,this._dataApi.queryEquals(Item,"typeId",subId)));
            }
         }
         for each(id in this._currentFilteredIds)
         {
            if(this._subFilteredTypes.indexOf(id) == -1)
            {
               ...
               {
                  typeResult = this._dataApi.queryIntersection(this._allItemsWithCurrentTypeIds,this._dataApi.queryUnion(typeResult,this._dataApi.queryEquals(Item,"typeId",this._dataApi.queryEquals(ItemType,"superTypeId",id))));
               }
            }
         }
      ...
      {
         var characId:int = 0;
         var mountWithEffect:* = null;
         var effectId:int = 0;
         var weaponEffectId:int = 0;
         var consumableEffect:int = 0;
         var effectIds:* = null;
         var mountWithCurrentEffect:* = null;
         var id:int = 0;
         var mount:* = null;
         ...
         var resultItemWithEffect:Object = this._allItemsWithCurrentTypeIds;
         if(this._currentCategory == this.EQUIPMENT_CATEGORY)
         {
         ...
         for each(characId in this._currentCharacIds)
         {
            ...
            switch(characId)
            {
               case this.CHARACID_BOOST_DEALT_DAMAGE_DISTANCE:
                  effectIds.push(ActionIdEnum.ACTION_CHARACTER_BOOST_DEALT_DAMAGE_PERCENT_MULTIPLIER_DISTANCE);
                  break;
               case this.CHARACID_DEBOOST_DEALT_DAMAGE_DISTANCE:
                  effectIds.push(ActionIdEnum.ACTION_CHARACTER_DEBOOST_RECEIVED_DAMAGE_PERCENT_MULTIPLIER_DISTANCE);
                  break;
               case this.CHARACID_BOOST_DEALT_DAMAGE_WEAPON:
                  effectIds.push(ActionIdEnum.ACTION_CHARACTER_BOOST_DEALT_DAMAGE_PERCENT_MULTIPLIER_WEAPON);
                  break;
               case this.CHARACID_BOOST_DEALT_DAMAGE_SPELLS:
                  effectIds.push(ActionIdEnum.ACTION_CHARACTER_BOOST_DEALT_DAMAGE_PERCENT_MULTIPLIER_SPELLS);
                  break;
               case this.CHARACID_BOOST_DEALT_DAMAGE_MELEE:
                  effectIds.push(ActionIdEnum.ACTION_CHARACTER_DEBOOST_RECEIVED_DAMAGE_PERCENT_MULTIPLIER_MELEE);
                  break;
               case this.CHARACID_DEBOOST_DEALT_DAMAGE_MELEE:
                  effectIds.push(ActionIdEnum.ACTION_CHARACTER_BOOST_DEALT_DAMAGE_PERCENT_MULTIPLIER_MELEE);
                  break;
               default:
      ...
      public function filterTargetEquipment() : Vector.<uint>
      {
         var itemId:int = 0;
         var item:* = null;
         var targetEquipment:int = 0;
         var effect:* = null;
         if(!this._allItemsWithCurrentTypeIds)
         {
            return new Vector.<uint>();
         ...
         var countItemsWithCurrentType:uint = this._allItemsWithCurrentTypeIds.length;
         var countTargetEquipment:uint = this._currentCharacIds.length;
         for(itemId = 0; itemId < countItemsWithCurrentType; itemId++)
         {
            item = this._dataApi.getItem(this._allItemsWithCurrentTypeIds[itemId]);
            for(targetEquipment = 0; targetEquipment < countTargetEquipment; targetEquipment++)
            {
               for each(effect in item.possibleEffects)
               {
                  if((effect.effectId == ActionIdEnum.ACTION_WRAPPER_OBJECT_CATEGORY || effect.effectId == ActionIdEnum.ACTION_LIVING_OBJECT_CATEGORY) && effect.parameter2 == this._currentCharacIds[targetEquipment])
                  {
      ...
      {
         this.setLevelRange(this._minLevel,this._maxLevel);
      }
   }
}
