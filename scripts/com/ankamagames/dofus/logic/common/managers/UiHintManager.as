package com.ankamagames.dofus.logic.common.managers
{
   ...
   public class UiHintManager
   {
      ...
      private static var shPreview:SubhintComponent;
      
      private static var _elementUsedHandCursor:Boolean;
      ...
      public static function createSubHints(target:UiRootContainer, uiModule:UiModule, tab:String = "", guidedTuto:Boolean = true) : void
      {
         ...
         var uiRoot:* = null;
         var j:int = 0;
         var copyOfCurrentSubhintData:* = null;
         var button:* = null;
         ...
         for(var i:int = 0; i < _allTargets.length; )
         {
         ...
         if(subHints.length > 0)
         {
            ...
            OptionManager.getOptionManager("dofus")["resetUIHints"] = true;
         }
         else
         {
            _allTargets = null;
      ...
      public static function subhintPreview(data:Object, target:UiRootContainer, uiModule:UiModule) : void
      {
         ...
         shPreview = new SubhintComponent(1,subhintWrapper,uiModule,cont,subhintWrapper.hint_tooltip_guided,rect,myLocalToGlobal(new Point(cont.x,cont.y),cont,root).x - root.x + anchor.x + subhintWrapper.hint_position_x,myLocalToGlobal(new Point(cont.x,cont.y),cont,root).y - root.y + anchor.y + subhintWrapper.hint_position_y);
         root.addChild(shPreview);
         shPreview.showSubHint(true);
         shPreview.highLight(true);
      }
      ...
      public static function closeSubhintPreview() : void
      {
         ...
         {
            TooltipManager.hide("tuto_interface");
            shPreview.highLight(false);
            if(shPreview.parent)
            {
               shPreview.parent.removeChild(shPreview);
            }
            shPreview = null;
         }
      }
