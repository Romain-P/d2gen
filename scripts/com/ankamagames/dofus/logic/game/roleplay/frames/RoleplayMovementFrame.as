package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class RoleplayMovementFrame implements Frame
   {
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         var gmmcmsg:* = null;
         var canceledMoveMessage:* = null;
         var stackFrame:* = null;
         var moveBehavior:* = null;
         switch(true)
         {
            ...
            case msg is GameMapMovementMessage:
               ...
               else
               {
                  ...
                  if(this._lastMoveEndCellId != this._lastPlayerValidatedPosition.cellId)
                  {
                     ...
                     requestedPath = Pathfinding.findPath(DataMapProvider.getInstance(),playerEntity.position,this._lastPlayerValidatedPosition,!PlayedCharacterManager.getInstance().restrictions.cantWalk8Directions,true,null);
      ...
      function askMoveTo(cell:MapPoint) : Boolean
      {
         ...
         if(PlayedCharacterManager.getInstance().state == PlayerLifeStatusEnum.STATUS_TOMBSTONE)
         {
            return false;
         }
         ...
         var requestedPath:MovementPath = Pathfinding.findPath(DataMapProvider.getInstance(),playerEntity.position,cell,!PlayedCharacterManager.getInstance().restrictions.cantWalk8Directions,true,this.sendPath);
         return true;
      }
      ...
      private function sendPath(path:MovementPath) : void
      {
         ...
         if(OptionManager.getOptionManager("dofus")["enableForceWalk"] == true && (this._nextMovementBehavior == AtouinConstants.MOVEMENT_WALK || this._nextMovementBehavior == 0 && (ShortcutsFrame.ctrlKeyDown || SystemManager.getSingleton().os == OperatingSystem.MAC_OS && ShortcutsFrame.altKeyDown)))
         {
