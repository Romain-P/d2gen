package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class HavenbagFrame implements Frame
   {
      ...
      public function pushed() : Boolean
      {
         ...
         this._lotteryApi = new KardApi(client);
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         var hema:* = null;
         var hbfmsg:* = null;
         var furnitureSprites:* = null;
         var packedFurnitures:* = null;
         var ohbfsrmsg:* = null;
         var chbfsrmsg:* = null;
         var hfsa:* = null;
         var mcmsg:* = null;
         var clickedFurniture:* = null;
         var mrumsg:* = null;
         var momsg:* = null;
         var htsa:* = null;
         var ctrmsg:* = null;
         ...
         switch(true)
         {
            ...
            case msg is HavenbagSaveAction:
               ...
               for(k = 0; k < packedFurnitures.furnitureCellIds.length; k = k + ProtocolConstantsEnum.MAX_FURNITURES_PER_PACKET)
               {
            ...
            case msg is HavenBagDailyLoteryMessage:
               ...
               if(hbdlmsg.returnType == HavenBagDailyLoteryErrorEnum.HAVENBAG_DAILY_LOTERY_OK && this._lotteryApi.hasOwnProperty("consumeByIdApiCall"))
               {
                  this._consumeCallback = new Callback(this._lotteryApi["consumeByIdApiCall"],XmlConfig.getInstance().getEntry("config.lang.current"),parseFloat(hbdlmsg.gameActionId),GameID.current);
                  HaapiKeyManager.getInstance().removeEventListener(ApiKeyReadyEvent.READY,this.onApiKeyReady);
      ...
      private function onApiKeyReady(pEvent:ApiKeyReadyEvent) : void
      {
         this._lotteryApi.getApiClient().setApiKey(pEvent.haapiKey);
      ...
      {
         var kard:* = null;
         if(pEvent.result is Array && pEvent.result.length == 1 && pEvent.result[0] is KardKard)
         {
            kard = pEvent.result[0] as KardKard;
            KernelEventsManager.getInstance().processCallback(HookList.HavenBagLotteryGift,kard.description);
         }
      }
      ...
      private function onError(pEvent:ApiClientEvent) : void
      {
         ...
         _log.error(pEvent.errorMsg);
         throw new Error(pEvent.errorMsg);
      }
