package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class RoleplayInteractivesFrame implements Frame
   {
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case msg is InteractiveElementMouseOverMessage:
               if(OptionManager.getOptionManager("dofus")["enableForceWalk"] == true && (ShortcutsFrame.ctrlKeyDown || SystemManager.getSingleton().os == OperatingSystem.MAC_OS && ShortcutsFrame.altKeyDown))
               {
                  return false;
               }
      ...
      public function getUseDirection(user:TiphonSprite, useAnimation:String, worldPos:MapPoint) : uint
      {
         ...
         {
            useDirection = uint(user.getDirection());
         }
         else
         {
            useDirection = uint((user as IMovable).position.advancedOrientationTo(worldPos,true));
         }
