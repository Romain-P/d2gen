   ...
   public class TimeManager implements IDestroyable
   {
      ...
      public function getDuration(time:Number, short:Boolean = false, addSeconds:Boolean = false) : String
      {
         ...
         if(!short)
         {
            if(showSeconds)
            {
               second = nsecond + " " + PatternDecoder.combine(this._nameSeconds,"f",nsecond <= 1);
            }
            minute = nminute + " " + PatternDecoder.combine(this._nameMinutes,"f",nminute <= 1);
            hour = nhour + " " + PatternDecoder.combine(this._nameHours,"f",nhour <= 1);
            day = nday + " " + PatternDecoder.combine(this._nameDays,"f",nday <= 1);
            month = nmonth + " " + PatternDecoder.combine(this._nameMonths,"f",nmonth <= 1);
            year = nyear + " " + PatternDecoder.combine(this._nameYears,"f",nyear <= 1);
            if(nyear != 0)
            {
               if(nmonth != 0)
               {
