   ...
   public class MoveBehavior extends AbstractBehavior
   {
      ...
      override public function processInputMessage(pMsgToProcess:Message, pMode:String) : Boolean
      {
         ...
         if((pMsgToProcess is CellClickMessage || pMsgToProcess is EntityClickMessage) && !PlayedCharacterManager.getInstance().isFighting && pMode == type)
         {
            ...
            {
               tmpCellId = uint((pMsgToProcess as CellClickMessage).cellId);
            }
            ...
            {
               tmpCellId = uint((pMsgToProcess as EntityClickMessage).entity.position.cellId);
            }
            else
            {
               return false;
            ...
            if(entity != null && entity.position.cellId != tmpCellId)
            {
               ...
               this.forceWalk = OptionManager.getOptionManager("dofus")["enableForceWalk"] == true && (ShortcutsFrame.ctrlKeyDown || SystemManager.getSingleton().os == OperatingSystem.MAC_OS && ShortcutsFrame.altKeyDown);
               return true;
            }
         }
