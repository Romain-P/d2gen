package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class PartyManagementFrame implements Frame
   {
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case msg is PartyRefuseInvitationNotificationMessage:
               ...
               {
                  for(iMember = 0; iMember < this._arenaPartyMembers.length; )
                  {
               ...
               {
                  for(iMember = 0; iMember < this._partyMembers.length; )
                  {
            ...
            case msg is PartyCancelInvitationNotificationMessage:
               ...
               {
                  for(iMember2 = 0; iMember2 < this._arenaPartyMembers.length; )
                  {
               ...
               {
                  for(iMember2 = 0; iMember2 < this._partyMembers.length; )
                  {
            ...
            case msg is PartyMemberRemoveMessage:
               ...
               {
                  for(iMember3 = 0; iMember3 < this._arenaPartyMembers.length; )
                  {
               ...
               {
                  for(iMember3 = 0; iMember3 < this._partyMembers.length; )
                  {
      ...
      {
         var playerAlignmentInfos:* = null;
         var member:* = null;
         var restrictionInfo:* = null;
      ...
      private function onTimerTick(pEvent:TimerEvent) : void
      {
         var member:* = null;
         var playerLPTM:* = null;
         var additionalLifePoint:* = 0;
         var newLifePoints:* = 0;
         var index:* = undefined;
         var lptm:* = null;
         var playerLPTM2:* = null;
         var additionalLifePoint2:* = 0;
         var newLifePoints2:* = 0;
         var lptm2:* = null;
         for each(member in this._partyMembers)
         {
            if(member.lifePoints < member.maxLifePoints && member.regenRate > 0)
            {
               ...
               newLifePoints = uint(playerLPTM.originalLifePoint + additionalLifePoint);
               if(newLifePoints >= member.maxLifePoints)
               {
                  newLifePoints = uint(member.maxLifePoints);
               }
         ...
         for each(member in this._arenaPartyMembers)
         {
            if(member.lifePoints < member.maxLifePoints && member.regenRate > 0)
            {
               ...
               newLifePoints2 = uint(playerLPTM2.originalLifePoint + additionalLifePoint2);
               if(newLifePoints2 >= member.maxLifePoints)
               {
                  newLifePoints2 = uint(member.maxLifePoints);
               }
