package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class AbstractEntitiesFrame implements Frame
   {
      ...
      public function addOrUpdateActor(infos:GameContextActorInformations, animationModifier:IAnimationModifier = null) : AnimatedCharacter
      {
         ...
         characterEntity.visibleAura = !characterEntity.isMoving && OptionManager.getOptionManager("tiphon").auraMode >= OptionEnum.AURA_ALWAYS;
      ...
      public function addEntityIcon(pEntityId:Number, pIconName:String, pIconCategory:int = 0, offsetX:Number = 0, offsetY:Number = 0) : void
      {
      ...
      public function removeIcon(pEntityId:Number, pIconName:String = null, pForce:Boolean = false) : void
      {
         ...
         if(this._entitiesIconsCounts[pEntityId])
         {
            ...
            else
            {
               ...
               {
                  if(this._entitiesIconsNames[pEntityId][cat] && this._entitiesIconsNames[pEntityId][cat].length > 0)
                  {
                     for(i = 0; i < this._entitiesIconsNames[pEntityId][cat].length; )
                     {
      ...
      public function getIconEntityBounds(pEntity:AnimatedCharacter) : IRectangle
      {
      ...
      protected function showIcons(pEvent:Event = null) : void
      {
         ...
         var isCarried:Boolean = false;
         var entitiesToDelete:Array = new Array();
         for(entityId in this._entitiesIconsNames)
         {
            ...
            else
            {
               ...
               if(this._updateAllIcons || entity.getAnimation() && entity.getAnimation().indexOf(AnimationEnum.ANIM_STATIQUE) == -1 || !this._entitiesIcons[entityId] || this._entitiesIcons[entityId].needUpdate)
               {
                  ...
                  targetBounds = this.getIconEntityBounds(entity);
               }
               if(targetBounds)
               {
                  ...
                  for(iconCat in this._entitiesIconsNames[entityId])
                  {
                     for each(iconName in this._entitiesIconsNames[entityId][iconCat])
                     {
                        if(!ei.hasIcon(iconName))
                        {
                           ...
                           {
                              ei.addIcon(ICONS_FILEPATH_STATE + "" + iconName,iconName,this._entitiesIconsOffsets[iconName]);
                           }
                  ...
                  if(!newIcon)
                  {
                     ...
                     isCarried = entity.parentSprite && entity.parentSprite.carriedEntity == entity;
                     if(!entity || !entity.parent || !entity.displayed || isCarried)
                     {
                        if(ei.parent)
                        {
