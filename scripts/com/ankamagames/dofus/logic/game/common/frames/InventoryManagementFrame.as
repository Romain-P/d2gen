package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   import com.ankamagames.dofus.network.messages.game.presets.CharacterPresetSaveRequestMessage;
   ...
   import com.ankamagames.dofus.network.types.game.presets.CharacterBuildPreset;
   ...
   public class InventoryManagementFrame implements Frame
   {
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         var cpsrmsg:CharacterPresetSaveRequestMessage = null;
         ...
         var buildPreset:CharacterBuildPreset = null;
         ...
         var newBuild:CharacterBuildPreset = null;
         ...
         switch(true)
         {
            ...
            case msg is ObjectAddedMessage:
               ...
               if(oam.origin == 1)
               {
                  ...
                  message = PatternDecoder.combine(I18n.getUiText("ui.quest.itemObtained",[oam.object.quantity,"{item," + oam.object.objectGID + "," + oam.object.objectUID + "}"]),"n",oam.object.quantity <= 1);
            ...
            case msg is ObjectQuantityMessage:
               ...
               if(oqm.origin == 1)
               {
                  ...
                  if(quantity > 0)
                  {
                     ...
                     content = PatternDecoder.combine(I18n.getUiText("ui.quest.itemObtained",[quantity,"{item," + item.objectGID + "," + oqm.objectUID + "}"]),"n",quantity <= 1);
            ...
            case msg is PresetsMessage:
               ...
               for(iPreset = 0; iPreset < presetsCount; )
               {
                  ...
                  else if(preset is CharacterBuildPreset)
                  {
                     buildPreset = preset as CharacterBuildPreset;
                     ...
                     presetWrappers.push(BuildWrapper.create(buildPreset.id,-1,buildPreset.name,buildPreset.iconId,fullStatsPreset,itemsPreset,spellsPreset,true));
            ...
            case msg is PresetDeleteResultMessage:
               ...
               if(pdrmsg.code == PresetDeleteResultEnum.PRESET_DEL_OK)
               {
                  ...
                  if(deletePresetTypeId == CHARACTER_BUILD_PRESET_TYPE)
                  {
                     builds = InventoryManager.getInstance().builds;
                     for each(build in builds)
                     {
                  ...
                  else if(deletePresetTypeId == IDOLS_PRESET_TYPE)
                  {
                     ...
                     for each(preset in idolsPreset)
                     {
                        if(preset.id == pdrmsg.presetId)
                        {
                           presetIndex = idolsPresets.indexOf(preset);
                           break;
                        }
                     }
            ...
            case msg is CharacterPresetSaveRequestAction:
               ...
               cpsrmsg = new CharacterPresetSaveRequestMessage();
               cpsrmsg.initCharacterPresetSaveRequestMessage(cpsra.presetId,cpsra.symbolId,cpsra.fullSave,cpsra.name);
               ConnectionsHandler.getConnection().send(cpsrmsg);
               return true;
            ...
            case msg is PresetSavedMessage:
               ...
               else if(psmsg.preset is CharacterBuildPreset)
               {
                  newBuild = psmsg.preset as CharacterBuildPreset;
                  ...
                  if(this._presetTypeIdByPresetId[newBuild.id])
                  {
                     ...
                     for each(build in builds)
                     {
                        ...
                        {
                           build.updateBuild(newBuild.name,newBuild.iconId,fullStatsPresetSaved,itemsPresetSaved,spellsPresetSaved);
                        }
                     }
                  }
                  else
                  {
                     InventoryManager.getInstance().builds.push(BuildWrapper.create(newBuild.id,-1,newBuild.name,newBuild.iconId,fullStatsPresetSaved,itemsPresetSaved,spellsPresetSaved,true));
      ...
      private function useItem(oua:ObjectUseAction, iw:ItemWrapper) : void
      {
         ...
         var roleplayMovementFrame:* = null;
      ...
      private function getShortcutWrapperPropFromShortcut(shortcut:Shortcut) : Object
      {
         ...
         {
            id = uint((shortcut as ShortcutObjectItem).itemUID);
            gid = uint((shortcut as ShortcutObjectItem).itemGID);
            shortcutType = uint(DataEnum.SHORTCUT_TYPE_ITEM);
         }
         else if(shortcut is ShortcutObjectPreset)
         {
            id = uint((shortcut as ShortcutObjectPreset).presetId);
            shortcutType = uint(DataEnum.SHORTCUT_TYPE_PRESET);
         }
         else if(shortcut is ShortcutSpell)
         {
            id = uint((shortcut as ShortcutSpell).spellId);
            shortcutType = uint(DataEnum.SHORTCUT_TYPE_SPELL);
         }
         else if(shortcut is ShortcutSmiley)
         {
            id = uint((shortcut as ShortcutSmiley).smileyId);
            shortcutType = uint(DataEnum.SHORTCUT_TYPE_SMILEY);
         }
         else if(shortcut is ShortcutEmote)
         {
            id = uint((shortcut as ShortcutEmote).emoteId);
            shortcutType = uint(DataEnum.SHORTCUT_TYPE_EMOTE);
         }
         else if(shortcut is ShortcutObjectIdolsPreset)
         {
            id = uint((shortcut as ShortcutObjectIdolsPreset).presetId);
            shortcutType = uint(DataEnum.SHORTCUT_TYPE_IDOLS_PRESET);
         }
         return {
            "id":id,
            "gid":gid,
