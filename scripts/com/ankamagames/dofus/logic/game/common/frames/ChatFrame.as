package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   import com.ankamagames.dofus.datacenter.communication.Smiley#98;
   ...
   import com.ankamagames.dofus.datacenter.notifications.Notification#318;
   ...
   import com.ankamagames.dofus.logic.game.spin.chat.SpinChatConnection;
   ...
   public class ChatFrame implements Frame
   {
      ...
      private var _spinChatConnection:SpinChatConnection;
      ...
      public function pushed() : Boolean
      {
         ...
         for each(pack in this._aSmileyPacks)
         {
            ...
            {
               smiley = Smiley#98.getSmileyById(smileypId);
               if(smiley)
               {
         ...
         if(!chatOpt["favoriteSmileys"])
         {
            chatOpt.add("favoriteSmileys",[]);
         }
         ...
         this._spinChatConnection = new SpinChatConnection();
         if(BuildInfos.BUILD_TYPE >= BuildTypeEnum.BETA)
         {
            this._spinChatConnection.connect();
         }
         return true;
      }
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         var soldLot:ObjectItemGenericQuantityPrice = null;
         ...
         switch(true)
         {
            ...
            case msg is ChatServerMessage:
               ...
               this.saveMessage(csmsg.channel,msgContent,content,this.getRealTimestamp(csmsg.timestamp),csmsg.fingerprint,csmsg.senderId,csmsg.senderName);
            ...
            case msg is NotificationByServerMessage:
               ...
               notification = Notification#318.getNotificationById(nbsmsg.id);
            ...
            case msg is ExchangeBidHouseUnsoldItemsMessage:
               ...
               {
                  this._unsoldItems[this._numUnsoldItemsLinks].push(OfflineSaleWrapper.create(i + 1,OfflineSaleWrapper.TYPE_UNSOLD,ebhuimsg.items[i].objectGID,ebhuimsg.items[i].quantity,NaN));
               }
               unsoldItemsMsg = "{offlineSales,1," + this._numUnsoldItemsLinks + "::" + PatternDecoder.combine(I18n.getUiText("ui.sell.unsoldItemsLink",[nbUnsoldLots]),"n",nbUnsoldLots <= 1) + "}";
            ...
            case msg is ExchangeOfflineSoldItemsMessage:
               ...
               {
                  this._offlineSales.push(OfflineSaleWrapper.create(this._offlineSales.length + 1,OfflineSaleWrapper.TYPE_BIDHOUSE,soldLot.objectGID,soldLot.quantity,soldLot.price));
               }
               for each(soldLot in eosimsg.merchantItems)
               {
                  this._offlineSales.push(OfflineSaleWrapper.create(this._offlineSales.length + 1,OfflineSaleWrapper.TYPE_MERCHANT,soldLot.objectGID,soldLot.quantity,soldLot.price));
               }
               totalKamas = 0;
               ...
               offlineSalesMsg = "{offlineSales,0,0::" + PatternDecoder.combine(I18n.getUiText("ui.sell.offlineSales",[nbOfflineSales,StringUtils.kamasToString(totalKamas,"")]),"n",nbOfflineSales <= 1) + "}";
      ...
      {
         this._spinChatConnection.disconnect();
         return true;
      }
      
      public function get spinChatConnection() : SpinChatConnection
      {
         return this._spinChatConnection;
      }
      ...
      private function saveMessage(channel:int = 0, baseContent:String = "", content:String = "", timestamp:Number = 0, fingerprint:String = "", senderId:Number = 0, senderName:String = "", objects:Vector.<ItemWrapper> = null, receiverName:String = "", receiverId:Number = 0, textKey:uint = 0, params:Array = null, admin:Boolean = false) : void
      {
         var sentence:* = null;
         var max:* = 0;
         var i:int = 0;
         ...
         {
            max = uint(this._aMessagesByChannel[channel].length - this._maxMessagesStored);
            for(i = 0; i < max; i++)
            {
               removedSentences++;
            }
      ...
      {
         var wordl:* = null;
         var nAddWarning:int = 0;
         var newContent:* = null;
         var lang:* = null;
         var finalText:* = null;
         var indexOfMethod:* = false;
         var punctuationRegExp:* = null;
         var wordsToCheck:* = null;
         var wordi:* = null;
         var finalWord:* = null;
         ...
         var safeReplace:* = null;
         var upperContent:* = null;
