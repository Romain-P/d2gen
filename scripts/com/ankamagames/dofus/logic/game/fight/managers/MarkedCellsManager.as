   ...
   public class MarkedCellsManager implements IDestroyable
   {
      ...
      public function addMark(markId:int, markType:int, associatedSpell:Spell, associatedSpellLevel:SpellLevel, cells:Vector.<GameActionMarkedCell>, teamId:int = 2, markActive:Boolean = true, markImpactCellId:int = -1) : void
      {
         ...
         if(!this._marks[markId] || this._marks[markId].cells.length == 0)
         {
            ...
            if(cells.length > 0)
            {
               ...
               selectionStrata = uint(!!Atouin.getInstance().options.transparentOverlayMode?uint(PlacementStrataEnums.STRATA_NO_Z_ORDER):markType == GameActionMarkTypeEnum.PORTAL?uint(PlacementStrataEnums.STRATA_PORTAL):uint(PlacementStrataEnums.STRATA_GLYPH));
      ...
      public function removeGlyph(markId:int) : void
      {
         ...
         if(this._glyphs[markId])
         {
            ...
            {
               for(i = 0; i < this._glyphs[markId].length; )
               {
      ...
      private function onPropertyChanged(pEvent:PropertyChangeEvent) : void
      {
         ...
         if(pEvent.propertyName == "transparentOverlayMode")
         {
            for(markId in this._marks)
            {
               ...
               {
                  strata = uint(PlacementStrataEnums.STRATA_NO_Z_ORDER);
               }
               else
               {
                  strata = uint(mi.markType == GameActionMarkTypeEnum.PORTAL?uint(PlacementStrataEnums.STRATA_PORTAL):uint(PlacementStrataEnums.STRATA_GLYPH));
               }
      ...
      private function updateDataMapProvider() : void
      {
         ...
         for(i = 0; i < AtouinConstants.MAP_CELLS_COUNT; )
         {
