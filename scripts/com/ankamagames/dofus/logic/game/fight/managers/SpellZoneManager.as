package com.ankamagames.dofus.logic.game.fight.managers
{
   ...
   public class SpellZoneManager implements IDestroyable
   {
      ...
      public function getSpellZone(spell:*, ignoreShapeA:Boolean = false, ignoreMaxSize:Boolean = true, spellImpactCell:int = 0, casterCell:int = 0, pForPreview:Boolean = true) : IZone
      {
         ...
         var zonesCells:* = null;
         var effect:* = null;
         var additionalZone:* = null;
         var additionalZoneCells:* = null;
         ...
         if(pForPreview && spell.default_zone)
         {
            ...
            shape = uint(effectZone.zoneShape);
            ray = uint(uint(effectZone.zoneSize));
            minRay = uint(uint(effectZone.zoneMinSize));
            stopAtTarget = uint(uint(effectZone.zoneStopAtTarget));
         }
         else if(!spell.hasOwnProperty("shape"))
         {
            for each(fx in spell.effects)
            {
               ...
               {
                  shape = uint(fx.zoneShape);
                  ray = uint(uint(fx.zoneSize));
                  minRay = uint(uint(fx.zoneMinSize));
                  stopAtTarget = uint(uint(fx.zoneStopAtTarget));
               }
            }
         }
         else
         {
            shape = uint(spell.shape);
            ray = uint(spell.ray);
         }
         ...
         if(pForPreview && spell.hasOwnProperty("additionalEffectsZones") && spell.additionalEffectsZones && spell.additionalEffectsZones.length)
         {
            ...
            for each(effectZone in spell.additionalEffectsZones)
            {
               ...
               for each(entityId in entitiesIds)
               {
                  ...
                  if(this.checkZone(entityInfos,shape,zonesCells) && DamageUtil.verifySpellEffectMask(CurrentPlayedFighterManager.getInstance().currentFighterId,entityId,effect,spellImpactCell))
                  {
                     ...
                     additionalZone.direction = MapPoint.fromCellId(casterCell).advancedOrientationTo(MapPoint.fromCellId(spellImpactCell));
