package com.ankamagames.dofus.logic.game.fight.managers
{
   ...
   public class TacticModeManager
   {
      ...
      public function TacticModeManager(pvt:PrivateClass#303)
      {
      ...
      public static function getInstance() : TacticModeManager
      {
         if(_self == null)
         {
            _self = new TacticModeManager(new PrivateClass#303());
         }
         return _self;
      }
      ...
      private function initTemplates() : void
      {
         ...
         loader.load(new Uri(Atouin.getInstance().options.tacticalModeTemplatesPath),null,TacticalTemplatesAdapter);
      }
      ...
      public function show(pPoint:WorldPointWrapper, openByTerminal:Boolean = false) : void
      {
         ...
         var useNewTacticalMode:Boolean = OptionManager.getOptionManager("dofus")["useNewTacticalMode"];
         ...
         if(useNewTacticalMode)
         {
            ...
            this._templateToShow = OptionManager.getOptionManager("dofus")["forceDefaultTacticalModeTemplate"] == true?1:uint(Math.max(templateId,1));
         ...
         MapDisplayManager.getInstance().hideBackgroundForTacticMode(true,!useNewTacticalMode?0:uint(template.backgroundColor));
         if(SHOW_BACKGROUND)
         {
            this.loadBackground();
         }
      ...
      private function clearUnneededCells(zonesMaxX:int, zonesMaxY:int, zonesMinX:int, zonesMinY:int) : void
      {
         ...
         var unreachablePathDuplicate:* = this._unreachablePath.concat();
         var len:int = unreachablePathDuplicate.length;
         for(i = 0; i < len; )
         {
            uCell = uint(unreachablePathDuplicate[i]);
      ...
      private function updateCellWithRealCellZone() : void
      {
         ...
         var collideZones:* = new Array();
         for(cellId = 0; cellId < len; )
         {
...
class PrivateClass#303
{
    
   
   function PrivateClass#303()
   {
      super();
   }
}
