   ...
   public final class CurrentPlayedFighterManager
   {
      ...
      public function canCastThisSpell(spellId:uint, lvl:uint, pTargetId:Number = 0, result:Array = null) : Boolean
      {
         ...
         if(spellId == 0 && player.currentWeapon != null)
         {
            ...
            maxCastPerTurn = uint(weapon.maxCastPerTurn);
         }
         else
         {
            apCost = uint(thisSpell.apCost);
            maxCastPerTurn = uint(thisSpell.maxCastPerTurn);
         }
         ...
         if(pTargetId != 0)
         {
            numberCastOnTarget = uint(spellManager.getCastOnEntity(pTargetId));
