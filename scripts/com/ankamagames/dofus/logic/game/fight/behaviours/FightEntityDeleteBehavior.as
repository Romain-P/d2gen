   ...
   public class FightEntityDeleteBehavior implements IEntityDeleteBehavior
   {
      ...
      public function entityDeleted(pEntity:AnimatedCharacter) : void
      {
         ...
         if(pEntity.rootEntity == pEntity)
         {
            ...
            if(casterInfos && !castFrame.hasInvocationPreview && !castFrame.isTeleportationPreviewEntity(pEntity.id) && !castFrame.isReplacementInvocation(pEntity) && selection && selection.cells && selection.cells.length && pEntity.position && SelectionManager.getInstance().isInside(pEntity.position.cellId,"SpellCastLos"))
            {
