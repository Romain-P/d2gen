package com.ankamagames.dofus.logic.game.fight.frames
{
   ...
   public class FightTurnFrame implements Frame
   {
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         var playerInfos:* = null;
         var imE:* = null;
         var scrmsg:* = null;
         var text:* = null;
         var ccmmsg:* = null;
         var spellCastFrame:* = null;
         ...
         switch(true)
         {
            ...
            case msg is GameFightSpellCastAction:
               ...
               if(bf && bf.turnsCount <= 0 || playerInformation && playerInformation.alive)
               {
            ...
            case msg is GameFightTurnFinishAction:
               ...
               if(this._remainingDurationSeconds > 0 && !playerInfos.stats.summoned)
               {
                  ...
                  {
                     KernelEventsManager.getInstance().processCallback(ChatHookList.TextInformation,PatternDecoder.combine(I18n.getUiText("ui.fight.secondsAdded",[secondsToReport]),"n",secondsToReport <= 1),ChatActivableChannelsEnum.PSEUDO_CHANNEL_INFO,TimeManager.getInstance().getTimestamp());
                  }
      ...
      {
         if(this._turnFinishingNoNeedToRedrawMovement || !Dofus.getInstance().options.showMovementArea)
         {
         ...
         var characteristicsMovementPointsCurrent:int = characteristics.movementPointsCurrent;
         var movementPoints:int = characteristicsMovementPointsCurrent;
      ...
      public function drawPath(cell:MapPoint = null) : void
      {
         ...
         var path:MovementPath = Pathfinding.findPath(DataMapProvider.getInstance(),this._playerEntity.position,cell,false,false,null,null,true);
         if(DataMapProvider.getInstance().obstaclesCells.length > 0 && (path.path.length == 0 || path.path.length > movementPoints))
         {
            path = Pathfinding.findPath(DataMapProvider.getInstance(),this._playerEntity.position,cell,false,false,null,null,true,false);
            if(path.path.length > 0)
            {
               pathLen = uint(path.path.length);
               for(i = 0; i < pathLen; )
               {
         ...
         if(this._cells.length)
         {
            ...
            {
               orientation = uint(mp.orientationTo(MapPoint.fromCellId(this._cells[this._cells.length - 1])));
               if(orientation % 2 == 0)
               {
                  orientation++;
               }
      ...
      private function askMoveTo(cell:MapPoint) : Boolean
      {
         ...
         this._cells.unshift(this._playerEntity.position.cellId);
         path.fillFromCellIds(this._cells.slice(0,this._cells.length - 1));
         path.start = this._playerEntity.position;
         path.end = MapPoint.fromCellId(this._cells[this._cells.length - 1]);
