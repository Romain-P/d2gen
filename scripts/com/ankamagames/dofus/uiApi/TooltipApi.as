package com.ankamagames.dofus.uiApi
{
   ...
   public class TooltipApi implements IApi
   {
      ...
      [Untrusted]
      public function place(target:*, showDirectionalArrow:Object, point:uint = 6, relativePoint:uint = 0, offset:* = 3, checkSuperposition:Boolean = false, cellId:int = -1, offsetRect:IRectangle = null, alwaysDisplayed:Boolean = true) : void
      {
         if(target && CheckCompatibility.isCompatible(IRectangle,target))
         {
            if(this._currentUi.ready)
            {
               this.placeTooltip(this._currentUi,target,showDirectionalArrow,point,relativePoint,offset,checkSuperposition,cellId,offsetRect,alwaysDisplayed);
            }
            else
            {
               this._currentUi.removeEventListener(UiRenderEvent.UIRenderComplete,this.onTooltipReady);
               this._ttCallbacks[this._currentUi] = new Callback(this.placeTooltip,this._currentUi,target,showDirectionalArrow,point,relativePoint,offset,checkSuperposition,cellId,offsetRect,alwaysDisplayed);
               this._currentUi.addEventListener(UiRenderEvent.UIRenderComplete,this.onTooltipReady);
            }
         }
      }
      
      private function placeTooltip(pTooltipUi:UiRootContainer, pTarget:*, showDirectionalArrow:Object, pPoint:uint, pRelativePoint:uint, pOffset:*, pCheckSuperposition:Boolean, pCellId:int, pOffsetRect:IRectangle, pAlwaysDisplayed:Boolean) : void
      {
         ...
         for each(ui in tooltipUis)
         {
            if(i == 0)
            {
               if(tooltipNames.length == 1)
               {
                  ...
                  {
                     this.placeTooltip(ui,sourceUi,false,LocationEnum.POINT_TOPRIGHT,LocationEnum.POINT_TOPLEFT,offset,false,0,null,true);
                     ui.x = ui.x - offset * 2;
                  }
                  else
                  {
                     this.placeTooltip(ui,sourceUi,false,LocationEnum.POINT_TOPLEFT,LocationEnum.POINT_TOPRIGHT,offset,false,0,null,true);
                  }
                  if(ui.y - 41 >= 0)
                  {
                     ui.y = ui.y - 41;
               ...
               if(sourceUi.x - ui.width - offset >= 0)
               {
                  this.placeTooltip(ui,sourceUi,false,LocationEnum.POINT_RIGHT,LocationEnum.POINT_LEFT,offset,false,0,null,true);
                  ui.x = ui.x - offset * 2;
               }
               else
               {
                  this.placeTooltip(ui,sourceUi,false,LocationEnum.POINT_LEFT,LocationEnum.POINT_RIGHT,offset,false,0,null,true);
               }
               mainColumnHeight = Number(mainColumnHeight + (ui.height + offset));
               mainColumnTooltips.push(ui);
               mainColumnY = Number(ui.y);
            }
            else if(i < 4)
            {
               ...
               if(goOnTop)
               {
                  this.placeTooltip(ui,lastUi,false,LocationEnum.POINT_BOTTOM,LocationEnum.POINT_TOP,offset,false,0,null,false);
                  mainColumnY = Number(ui.y);
               }
               else
               {
                  this.placeTooltip(ui,lastUi,false,LocationEnum.POINT_TOP,LocationEnum.POINT_BOTTOM,offset,false,0,null,false);
               }
               ui.x = lastUi.x;
               mainColumnHeight = Number(mainColumnHeight + (ui.height + offset));
               mainColumnTooltips.push(ui);
            ...
            else
            {
               ...
               if(goOnTop)
               {
                  this.placeTooltip(ui,lastUi,false,LocationEnum.POINT_BOTTOM,LocationEnum.POINT_TOP,offset,false,0,null,true);
               }
               else
               {
                  this.placeTooltip(ui,lastUi,false,LocationEnum.POINT_TOP,LocationEnum.POINT_BOTTOM,offset,false,0,null,true);
               }
               ui.x = lastUi.x;
            }
            lastUi = ui;
