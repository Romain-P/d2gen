package com.ankamagames.dofus.uiApi
{
   ...
   public class MapApi implements IApi
   {
      ...
      public function getShortcuts(worldMap:int) : Array
      {
         ...
         var result:* = [];
         for each(edge in WorldPathFinder.getWorldGraph().getEdges())
         {
            from = this.getMapPositionById(edge.from.mapId);
            to = this.getMapPositionById(edge.to.mapId);
            if(from != null && to != null && from.worldMap == worldMap && to.worldMap == worldMap)
            {
               ...
               if(manhattanDistance > 1)
               {
                  result.push({
                     "from":from,
                     "to":to,
                     "fromRp":edge.from.zoneId,
                     "toRp":edge.to.zoneId,
                     "transitionText":edge.transitions.length + "/" + edge.transitions[0].toString()
                  });
               }
            }
         }
      ...
      private function getSearchResultEntry(pId:int, pType:uint, pSearch:String) : Object
      {
         ...
         var i:int = 0;
         var j:int = 0;
         var strNoAccent:* = null;
         var nextIndex:int = 0;
         var numReplaced:int = 0;
         var hint:* = null;
         var subArea:* = null;
         var monster:* = null;
         var monsterSubAreasIds:* = null;
         var item:* = null;
         var subAreasIds:* = null;
         var resourceSubAreaIds:* = null;
         var hintName:* = null;
         var parent:* = null;
         var moreInfo:* = "";
         switch(pType)
      ...
      private function getItemValidSubAreas(pItem:Item) : SearchResult
      {
         ...
         var monster:* = null;
         var monsterId:int = 0;
         var monsterSubAreas:* = null;
         var monsterSubAreasResult:* = null;
         var resultCode:int = SearchResult.VALID_ALL;
