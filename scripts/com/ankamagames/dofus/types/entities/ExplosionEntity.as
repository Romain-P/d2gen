package com.ankamagames.dofus.types.entities
{
   ...
   public class ExplosionEntity extends Sprite implements IEntity
   {
      ...
      private function createParticle(container:DisplayObjectContainer, count:uint, transformColor:Array, type:uint, subExplosionRatio:Number, fxClass:Array, deathCallback:Function, xStart:Number = 0, yStart:Number = 0) : void
      {
         ...
         for(var i:int = 0; i < count; )
         {
            if(CURRENT_PARTICLES < MAX_PARTICLES)
            {
               ...
               if(type == TYPE_MIX)
               {
                  pType = uint(Math.random() > 0.5?uint(TYPE_TWIRL):uint(TYPE_CLASSIC));
               }
               if(transformColor)
               {
                  if(type == TYPE_MIX)
               ...
               _particules[p] = new TwirlParticle(p,100,Math.random() < subExplosionRatio,deathCallback,-container.parent.y + 20 * Math.random() - 10,pType == TYPE_TWIRL?10:uint(0));
               CURRENT_PARTICLES++;
            }
            i++;
         }
      ...
      private function onAdded(e:Event) : void
      {
         ...
         this.createParticle(this,this._particleCount,this._transformColor,this._type,!!this._explode?0.2:Number(0),this._fxClass,this.onParticuleDeath);
