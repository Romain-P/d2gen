package com.ankamagames.dofus.types.entities
{
   ...
   public class HavenbagFurnitureSprite extends Sprite implements IFurniture
   {
      ...
      private function reorderSubSprites() : void
      {
         ...
         for(var j:int = 1; j < this.cellsHeight; )
         {
            subSpriteOrder++;
      ...
      {
         var stripBounds:* = null;
         var totalStripsWidth:* = 0;
         var mp:* = null;
         var strip:* = null;
         var bd:* = null;
         var subSprite:* = null;
         var leftOverX:* = NaN;
         var leftOverOffset:* = NaN;
         var cellInBottomCorner:* = null;
         var i:int = 0;
         var j:int = 0;
         var gCellPos:* = null;
         var gContentPos:* = null;
         ...
         else
         {
            ...
            for(i = 0; i < this._data.cellsWidth; i++)
            {
               ...
               if(i == 0)
               {
                  ...
                  {
                     leftOverX = Number(leftOverX - stripBounds.width);
                     leftOverOffset = Number(leftOverOffset + (AtouinConstants.CELL_WIDTH - leftOverX));
                     continue;
                  }
                  leftOverOffset = Number(leftOverOffset + Math.floor(AtouinConstants.CELL_HALF_WIDTH + this._content.x));
               }
               ...
               leftOverX = 0;
               totalStripsWidth = uint(totalStripsWidth + stripBounds.width);
               stripBounds.width++;
               ...
               strip.bitmapData = bd;
               strip.smoothing = Atouin.getInstance().options.useSmooth;
